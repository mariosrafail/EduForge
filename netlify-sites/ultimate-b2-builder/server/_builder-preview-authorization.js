...
function normalizeIntent(input) {
  if (!exact(input, ["bookSlug", "componentSlug", "view", "pageId", "activityId", "releaseId"])) throw new Error("Invalid Builder preview authorization intent");
  ...
  if ((input.view === "page") !== Boolean(pageId) || (input.view === "activity") !== Boolean(activityId)
    || (pageId && !SAFE_ID.test(pageId)) || (activityId && !SAFE_ID.test(activityId)) || (releaseId && !UUID.test(releaseId))) throw new Error("Invalid Builder preview authorization intent");
  return { bookSlug: input.bookSlug, componentSlug: input.componentSlug, view: input.view, pageId, activityId, releaseId };
}
...
export function issueBuilderPreviewAuthorization(input, { environment = process.env, now = Date.now(), nonce = randomBytes(16).toString("base64url") } = {}) {
  ...
  const payload = Buffer.from(JSON.stringify({ version: 2, expiresAt: Math.floor(now / 1000) + builderPreviewAuthorizationTtlSeconds, nonce, actions: actionsFor(intent), ...intent }), "utf8").toString("base64url");
