...
The configured Functions directory is site-local and must not be changed to the repository-root `netlify/functions` directory. Its only deployable source entries are `builder-auth.js`, `builder-content.js`, `builder-open-response-import.js`, `builder-preview.js`, and `builder-teacher-ui-assets.js`; their implementation helpers live in the sibling `server/` directory and are bundled dependencies, not independently discoverable Functions. Authenticated routes remain `/builder/api/auth` plus the registered content and upload families. The separate GET-only public preview routes expose only their explicitly opted-in Student-safe review projections. Do not add LMS authentication, Platform Admin, or unrelated authoring mutation Functions to this site.
