...
- The authenticated hosted Builder loads a generic Book Library and component registry. Its first connected adapter is Ultimate B2 → Students Book. Hotspot, native Activity, supported Open Response, and UI Controller authoring use their registered revisioned APIs; Publication prepares and activates immutable component releases. One component-owned Review surface mounts the dedicated Viewer only while open. It has no `__hhplms` client. See `docs/hosted-book-builder-architecture.md`.
...
The configured Functions directory is site-local and must not be changed to the repository-root `netlify/functions` directory. Its deployable source entries are `builder-auth.js`, `builder-content.js`, `builder-native-activities.js`, `builder-native-preview.js`, `builder-open-response-import.js`, `builder-preview-authorization.js`, `builder-preview.js`, `builder-publication.js`, and `builder-teacher-ui-assets.js`; their implementation helpers live in the sibling `server/` directory and are bundled dependencies, not independently discoverable Functions. Authenticated routes remain `/builder/api/auth` plus the registered content, upload, native-activity, authorization, and publication families. The separate native preview Function is GET-only and accepts signed Viewer preview authorization, never Builder mutation requests. Public draft projections remain Student-safe; Teacher draft and release projections require the short-lived preview authorization or a valid Builder session. Do not add LMS authentication, Platform Admin, or unrelated authoring mutation Functions to this site.
...
The Viewer package keeps an empty tracked Functions directory and requires no `DATABASE_URL` or other database credential. Seven exact external 200 rewrite families proxy public draft content/import/assets, native draft documents/assets, and protected Teacher draft/release projections to the Builder site. They do not proxy Builder auth, authenticated mutation APIs, or direct Function paths, and Viewer fetches omit credentials. Protected requests carry only the short-lived, intent-scoped `previewAuthorization`; the Builder server validates its signature, expiry, action, component, release, and activity or authoritative page scope. A token protects server responses, never static bundle contents.

Bare `https://hhplms-viewer.netlify.app/#library` is canonical public mode: it starts from the checked-in public pack, committed hotspots, and tracked UI without any `/preview/` request, Teacher solution, native draft/asset request, Builder session, or preview authorization. An explicit Builder draft intent must contain a syntactically valid short-lived authorization; it performs one no-store hotspot request and an authorized Teacher UI request. Native hotspot activation then loads the separately protected current public/Teacher draft and managed assets within exact activity or authoritative page scope. An explicit exact-release intent loads the protected immutable public projection and fetches Teacher UI, legacy solutions, or native Teacher documents only when needed; it never consults the draft provider. Invalid explicit intents fail visibly and do not fall back to bare state. There is no polling, WebSocket, SSE, or direct database access.
