...
The first activation requires deploying the same commit to `hhplms-builder` first and `hhplms-viewer` second. After that, a Builder hotspot Save persists revision N and the Builder-created authorized Viewer URL reads revision N without committing the authoring JSON or redeploying the Viewer. Bare Viewer deliberately remains on the committed public baseline. For a manual draft smoke test, use the Builder preview frame or a newly issued Viewer URL; do not copy an expired token into documentation or logs.
