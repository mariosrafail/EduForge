...
The hosted runtime has three explicit valid modes. Bare/canonical Viewer uses committed public content, committed hotspots, tracked UI, and Student activity mode without any preview request. Authorized Builder preview loads the saved Student-safe draft/hotspot state and protected Teacher UI/Teacher import state. For a non-canonical native hotspot target it loads the current public native document from `/preview/native-activities/.../public`, renders Open Response, Image, or Single Choice with the shared native surfaces, and resolves managed artwork only through `/preview/native-activities/.../assets/<asset-id>`. Open Response model answers and Single Choice correct-option maps use the distinct protected `/teacher` route; Image makes no Teacher request. Page-scoped access succeeds only when the signed page and authoritative current native placement agree. Exact release preview remains independent: it loads a protected immutable public projection, then obtains Teacher UI, legacy solutions, and native Teacher documents only on demand from release routes. The draft provider does not activate in release mode. An invalid explicit preview fails closed; it never silently becomes bare mode.

Published `single-choice` activities use `native-response.v1` with stable question/option IDs. The Student projection never contains `correctAnswers` or `correctOptionId`. On assignment submission the server resolves the pinned immutable release, validates that each selected option belongs to its question, counts unanswered questions as incorrect, and persists the rounded percentage and counts. Teacher results may add feedback, but cannot override the server-owned score.
