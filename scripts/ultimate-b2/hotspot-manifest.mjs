...
const activityByKey = new Map(ultimateB2StudentsBookAuthoringActivities.map((activity) => [activity.activityKey, activity]));
...
function normalizeHotspot(hotspot, page, ids) {
...
export function validateAndNormalizeUltimateB2HotspotManifest(input) {
  ...
  for (const page of ultimateB2StudentsBookAuthoringPages) {
    ...
    const normalized = input.pages[page.id].map((hotspot) => normalizeHotspot(hotspot, page, ids));
