...
export function nativeDocumentPair(activityId, kind, pageId, title) {
  const interaction = kind === "mark-the-words" ? createEmptyNativeMarkWordsInteraction() : kind === "open-response" ? { kind, surface: { width: 1024, height: 582 }, artwork: [], questions: [] }
    : kind === "image" ? { kind, surface: { width: 1024, height: 582 }, images: [] }
      : kind === "complete-sentences" ? { kind, items: [], presentation: { kind: "image-hotspot", panels: [{ id: `panel-${"0".repeat(31)}1`, backgroundAssetSlot: "", sourceWidth: 1024, sourceHeight: 582, hotspots: [] }] } }
        ...
        : { kind, questions: [] };
  const solution = kind === "mark-the-words" ? { kind, answers: [] } : kind === "open-response" || kind === "listening" || kind === "oldschool-listening" ? { kind, modelAnswers: [] } : kind === "single-choice" ? { kind, correctAnswers: [] } : kind === "complete-sentences" ? { kind, answers: [] } : kind === "drag-drop" ? { kind, mappings: [] } : { kind };
