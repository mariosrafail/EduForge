...
export async function exerciseOldschoolQuestionVisibility(page) {
  await page.getByRole("button", { name: "Add Question" }).click(); await page.getByRole("textbox", { name: "Prompt", exact: true }).fill("What detail does the speaker emphasize?"); await page.getByRole("tab", { name: "Answer Key" }).click(); await page.getByLabel("Private model answer").fill("The speaker emphasizes the later printed detail."); await page.getByRole("tab", { name: "Visual" }).click(); await page.locator(".native-listening-question-authoring button.native-or-response").click({ force: true }); const oldschoolResponseControls = page.getByRole("group", { name: "Response region quick controls" }); await oldschoolResponseControls.getByLabel("Answer font").selectOption({ label: "Ahem" }); await oldschoolResponseControls.getByLabel("Quick Y").fill("440");
  ...
  await page.getByRole("button", { name: "Hide all prompts", exact: true }).click(); assert.equal(await page.locator(".native-listening-question-authoring .native-or-prompt").count(), 0); assert.equal(await page.locator(".native-listening-question-authoring .native-or-response").count(), 1);
  await page.getByRole("button", { name: "Restore all prompts", exact: true }).click(); assert.equal(await page.locator(".native-listening-question-authoring .native-or-prompt").count(), 1); await page.getByRole("button", { name: "Hide all prompts", exact: true }).click();
}
