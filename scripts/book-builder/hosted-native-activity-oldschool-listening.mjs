...
export async function exerciseOldschoolPersistentTranscript({ page, oldschoolId, nativeDocuments, publishViewerHotspot, screenshotRoot }) {
  ...
  await preview.locator("audio").evaluate((audio) => { Object.defineProperty(audio, "currentTime", { configurable: true, writable: true, value: 2.5 }); audio.dispatchEvent(new Event("timeupdate")); }); await page.waitForFunction((root) => root.querySelectorAll('.native-oldschool-listening-transcript-fragment[data-highlighted="true"]').length === 1, await preview.elementHandle()); await assertExactPresentation(preview, interaction, 1); await page.waitForTimeout(200); const laterScroll = await preview.locator(".native-oldschool-listening-page-viewport").evaluate((element) => element.scrollTop); assert.ok(laterScroll > 0); const scrollbar = preview.getByRole("scrollbar", { name: "Synchronized listening page scroll position" }); await scrollbar.waitFor(); const scrollPlacement = await scrollbar.evaluate((element) => { const control = element.getBoundingClientRect(); const stage = element.closest(".native-oldschool-listening-activity-stage").getBoundingClientRect(); return { right: stage.right - control.right, bottom: stage.bottom - control.bottom }; }); assert.ok(scrollPlacement.right >= 0 && scrollPlacement.right < 30 && scrollPlacement.bottom > 100, JSON.stringify(scrollPlacement));
