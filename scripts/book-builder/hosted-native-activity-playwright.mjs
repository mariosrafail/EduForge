...
try {
  ...
  await page.getByRole("button", { name: "Review", exact: true }).click(); let viewer = page.frameLocator(".unified-builder-review-dialog iframe"); await viewer.getByRole("button", { name: "Draft Open Response hotspot" }).waitFor(); assert.ok(authorizationIntents.length > firstReviewAuthorizationCount); assert.equal(await viewer.locator("title").evaluate((element) => element.ownerDocument.title), "Ultimate B2 Viewer");
