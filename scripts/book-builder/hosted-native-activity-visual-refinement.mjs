...
export async function exerciseCompleteSentencesGeometry(page, editor) {
  ...
  const frame = editor.getByRole("group", { name: "Blank hotspot selected", exact: true }); const beforeDrag = Number(await x.inputValue()); await frame.scrollIntoViewIfNeeded(); const box = await frame.boundingBox(); assert.ok(box);
