...
export async function exerciseCompleteSentencesGeometry(page, editor) {
  ...
  const frame = editor.getByRole("group", { name: "Blank hotspot selected", exact: true }); const beforeDrag = Number(await x.inputValue()); const grip = frame.locator(".studio-selection-label"); await grip.scrollIntoViewIfNeeded(); const box = await grip.boundingBox(); assert.ok(box);
  ...
  const resize = frame.getByRole("button", { name: "Resize Blank hotspot from bottom right" }); await resize.scrollIntoViewIfNeeded(); const resizeBox = await resize.boundingBox(); const beforeResize = Number(await width.inputValue()); assert.ok(resizeBox);
