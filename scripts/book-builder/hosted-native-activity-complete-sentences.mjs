...
export async function exerciseCompleteSentencesAuthoring({ page, nativeDocuments, nativeFonts, getNativeIndex, screenshotRoot, uploadReadableText, assertContentTabActive }) {
  ...
  assert.equal(savedCompletePresentation.panels[0].hotspots[0].presentation.fontSize, 240);
  assert.equal(savedCompletePresentation.panels[0].hotspots[0].presentation.color, "#e40083");
  assert.ok(savedCompletePresentation.panels[0].hotspots[0].presentation.fontAssetSlot);
