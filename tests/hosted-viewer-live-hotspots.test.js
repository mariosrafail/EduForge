...
test("Android runtime remains a bundled no-op with no preview route dependency", async () => {
  const [androidRuntime, vite, app] = await Promise.all([
  ...
  assert.match(app, /Promise\.all\(\[[\s\S]*interactiveContentPackProvider\.load\(\)[\s\S]*prepareUltimateB2StudentsBookHotspots\(\)/);
});
