...
test("Teacher editor uses section navigation, compact authoring modules, and the shared runtime preview", async () => {
  ...
  assert.match(bulk, /webkitdirectory/); assert.match(bulk, /Nothing is uploaded until/); assert.match(bulk, /Apply mappings/); assert.match(bulk, /Importing/); assert.match(bulk, /Needs review/); assert.doesNotMatch(bulk, /scan-directory|open-path|absolutePath/);
