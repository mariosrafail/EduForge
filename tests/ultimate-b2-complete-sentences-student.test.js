...
import authoring from "../src/data/ultimate-b2/authoring/unit-01-reading-exercise-4.complete-sentences.json" with { type: "json" };
import {
...
test("student Complete the Sentences derives its word bank and question identity from canonical authoring", () => {
  const words = completeSentencesWordBank(authoring);
  assert.equal(COMPLETE_SENTENCES_STUDENT_RESPONSE_SCHEMA_VERSION, 1);
  assert.equal(authoring.activityId, "ultimate-b2-sb-u1-p2-o4");
  assert.equal(authoring.example.answer, "On-demand");
  assert.equal(words.length, 8);
  assert.deepEqual(words.map(({ text }) => text), ["binge-watching", "season", "franchise", "episodes", "genre", "sub-plots", "Tuning in", "Media streaming"]);
  assert.deepEqual(words.map(({ questionId }) => questionId), Array.from({ length: 8 }, (_, index) => `ultimate-b2-sb-u1-p2-o4-q${index + 1}`));
  assert.equal(words.some(({ text }) => text === authoring.example.answer), false, "the solved example is not draggable");
});

test("word moves are one-use, replaceable, movable between blanks and returnable to the bank", () => {
  let answers = {};
  answers = moveCompleteSentencesWord(answers, authoring, "blank-2", "ultimate-b2-sb-u1-p2-o4-q1");
  assert.deepEqual(answers, { "ultimate-b2-sb-u1-p2-o4-q1": "binge-watching" });
  answers = moveCompleteSentencesWord(answers, authoring, "blank-2", "ultimate-b2-sb-u1-p2-o4-q2");
  assert.deepEqual(answers, { "ultimate-b2-sb-u1-p2-o4-q2": "binge-watching" }, "moving removes the previous placement");
  answers = moveCompleteSentencesWord(answers, authoring, "blank-3", "ultimate-b2-sb-u1-p2-o4-q2");
  assert.deepEqual(answers, { "ultimate-b2-sb-u1-p2-o4-q2": "season" }, "replacing returns the displaced word to the bank");
  answers = moveCompleteSentencesWord(answers, authoring, "blank-3");
...
test("progress reaches exactly 8/8 using the canonical server question keys", () => {
  let answers = {};
  for (const word of completeSentencesWordBank(authoring)) answers = moveCompleteSentencesWord(answers, authoring, word.id, word.questionId);
  assert.deepEqual(completeSentencesProgress(answers, authoring), { answered: 8, total: 8, complete: true });
