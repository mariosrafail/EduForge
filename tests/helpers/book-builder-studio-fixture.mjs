...
async function createUltimateProject(workspace, sourceRoot) {
  ...
  const candidates = Array.from({ length: 152 }, (_, index) => {
    ...
    confidence: 0.9, questions: [{ id: `question_fictional_${index}`, prompt: `Fictional question ${index + 1}`, promptAvailability: "structured", responseKind: "single-option", options: [{ id: `option_fictional_${index}_1`, order: 1, text: "Fictional option amber", textAvailability: "structured" }, { id: `option_fictional_${index}_2`, order: 2, text: "Fictional option blue", textAvailability: "structured" }] }],
