...
function harness() {
  const release = compileUltimateB2ComponentRelease();
  ...
  const handler = createBuilderPublicationHandler({
    ...
    loadRelease: async () => ({ id, release_number: 1, release_sha256: release.releaseSha256, runtime_compatibility_sha256: release.compatibility, source_snapshot: release.sourceSnapshot, source_snapshot_sha256: release.sourceSnapshotSha256, public_projection: release.publicProjection, public_projection_sha256: release.publicProjectionSha256, teacher_projection: release.teacherProjection, teacher_projection_sha256: release.teacherProjectionSha256, asset_manifest: release.assetManifest }), logger: { error() {} },
  });
...
test("release preview is pinned by strict component-owned UUID and exposes projections separately", async () => {
  ...
  const publicResponse = await handler(event(`${prefix}/public`, "GET", null, { cookie: "" }));
...
test("release preview rejects an asset manifest that is not exactly derived from its projections", async () => {
  ...
  const response = await handler(event(`/builder/preview/releases/books/ultimate-b2/components/ultimate-b2-students-book/${id}/public`, "GET", null, { cookie: "" }));
  assert.equal(response.statusCode, 409);
