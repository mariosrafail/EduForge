...
test("one sound fills only empty Unit assignments and usage survives save/reload", async (t) => {
  ...
  project = await store.save("sounds", { displayName: project.displayName, expectedRevision: project.revision, shell });
...
test("duplication is self-contained, revision 1, hash-identical, and omits exports/build state", async (t) => {
  ...
  const shell = structuredClone(source.shell); shell.background = image.asset.assetId; source = await store.save(source.projectId, { displayName: source.displayName, expectedRevision: source.revision, shell });
  ...
  assert.equal(duplicate.projectId, "ultimate-b4"); assert.equal(duplicate.displayName, "Ultimate B4"); assert.equal(duplicate.revision, 1); assert.deepEqual(duplicate.shell, source.shell); assert.deepEqual(duplicate.assets, source.assets); assert.equal((await store.load(source.projectId)).displayName, "Ultimate B3");
  const destination = path.join(workspace, "teacher-projects", duplicate.projectId); const copiedAsset = path.join(destination, ...image.asset.relativePath.split("/"));
  assert.deepEqual(await fs.readFile(copiedAsset), png); assert.equal((await fs.lstat(copiedAsset)).isSymbolicLink(), false); assert.deepEqual(await fs.readdir(path.join(destination, "exports")), []); await assert.rejects(() => fs.access(path.join(destination, ".build")));
...
test("referenced assets cannot be removed; replacement leaves a removable orphan", async (t) => {
  ...
  let shell = structuredClone(project.shell); shell.background = first.asset.assetId; project = await store.save("cleanup", { displayName: project.displayName, expectedRevision: project.revision, shell });
  ...
  shell = structuredClone(project.shell); shell.background = second.asset.assetId; project = await store.save("cleanup", { displayName: project.displayName, expectedRevision: project.revision, shell }); project = await store.removeAsset("cleanup", first.asset.assetId, project.revision);
...
test("shared sound removal stays blocked until every saved reference is cleared", async (t) => {
  ...
  project = await store.save(project.projectId, { displayName: project.displayName, expectedRevision: project.revision, shell });
  ...
  project = await store.save(project.projectId, { displayName: project.displayName, expectedRevision: project.revision, shell }); project = await store.removeAsset(project.projectId, imported.asset.assetId, project.revision);
