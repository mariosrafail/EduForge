...
function contentDecision(decisions, kind, targetId) {
  return list(decisions).find((item) => item.kind === kind && item.targetId === targetId) || null;
}

export function projectActivityContentField({ kind, targetId, detectedValue = null, availability = "unavailable", decisions = [] }) {
  const decision = contentDecision(decisions, kind, targetId);
  const manualUsable = decision?.approvalState === "approved" && !decision.stale && nonEmpty(decision.value);
  ...
  return {
    ...
    editorNote: decision?.editorNote || "",
...
export function projectEffectiveActivityContent(activity, decisions = []) {
  const activityId = activity.activityCandidateId || activity.activityId;
  const title = fieldProjection("activity_display_title", activityId, activity, decisions);
  const instructions = fieldProjection("activity_instruction_text", activityId, activity, decisions);
