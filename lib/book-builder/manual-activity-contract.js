...
export function serializeManualActivitiesArtifact(artifact) {
  ...
  const ids = new Set();
  for (const activity of artifact.activities) { const validation = validateManualActivity(activity, { requireApproval: activity.status === "approved" }); if (!validation.valid && activity.status === "approved") throw new Error(`Invalid manual activity: ${validation.errors.join("; ")}`); if (ids.has(activity.activityId)) throw new Error(`Duplicate manual activity: ${activity.activityId}`); ids.add(activity.activityId); }
