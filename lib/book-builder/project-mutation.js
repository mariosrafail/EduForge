...
import { ACTIVITY_CONTENT_DECISION_KINDS, projectActivityContentField } from "./activity-content-overrides.js";
...
export class ProjectMutationService {
  ...
  async preview(rawInput) {
    ...
    const contentOverride = ACTIVITY_CONTENT_DECISION_KINDS.has(input.kind) ? projectActivityContentField({
      kind: input.kind,
      targetId: input.targetId,
      detectedValue: context.targetSummary.detectedValue,
      availability: context.targetSummary.availability,
      decisions: [proposed],
    }) : null;
    return {
      ...
      ...(contentOverride ? { contentOverride: {
        field: context.targetSummary.field,
        detectedValue: contentOverride.detectedValue,
        proposedValue: proposed.value,
        effectiveValue: contentOverride.effectiveValue,
        valueOrigin: contentOverride.valueOrigin,
        availability: contentOverride.availability,
        characterCount: proposed.value.length,
        utf8ByteCount: Buffer.byteLength(proposed.value, "utf8"),
      } } : {}),
    };
  }
    ...
    return withProjectWriteLock({ workspace: this.workspace, projectId: this.projectId, sessionId: this.sessionId }, async () => {
      ...
      const record = {
        ...
        changedDecision: {
          ...
          valuePresent: typeof (nextDecision || currentDecision).value === "string" ? (nextDecision || currentDecision).value.length > 0 : (nextDecision || currentDecision).value !== null,
          valuePreview: typeof (nextDecision || currentDecision).value === "string" ? (nextDecision || currentDecision).value.replaceAll(/\s+/g, " ").slice(0, 120) : null,
        },
