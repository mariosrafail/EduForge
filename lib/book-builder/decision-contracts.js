...
import { ACTIVITY_CONTENT_DECISION_KINDS, ACTIVITY_CONTENT_TARGET_TYPES, normalizeActivityContentText } from "./activity-content-overrides.js";

export const DECISION_SCHEMA_VERSION = "1.0";
export const DECISION_APPROVAL_STATES = new Set(["draft", "approved", "rejected"]);
export const DECISION_TARGET_TYPES = new Set(["component", "page", "activity", "hotspot", "review", ...ACTIVITY_CONTENT_TARGET_TYPES]);
...
export function normalizeDecisionValue(kind, value, { allowedValues } = {}) {
  ...
  if (ACTIVITY_CONTENT_DECISION_KINDS.has(kind)) return normalizeActivityContentText(kind, value);
...
export function normalizeDecision(decision, facts = [], { requireCurrent = false } = {}) {
  ...
  if (current && ACTIVITY_CONTENT_DECISION_KINDS.has(decision.kind) && decision.approvalState === "approved" && !base.value) throw new Error(`${label} cannot approve an empty value`);
...
export function normalizeDecisionMutationInput(input) {
  ...
  return {
    targetId,
    kind: input.kind,
    value: ACTIVITY_CONTENT_DECISION_KINDS.has(input.kind)
      ? normalizeActivityContentText(input.kind, input.value, { allowEmpty: input.approvalState !== "approved" })
      : normalizeDecisionValue(input.kind, input.value),
