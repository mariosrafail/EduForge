  ...
  async summaries({ limit = 100 } = {}) {
    ...
    for (const name of names) {
      ...
      entries.push({
        ...
        valuePresent: item.changedDecision?.valuePresent === true,
        valuePreview: typeof item.changedDecision?.valuePreview === "string" ? item.changedDecision.valuePreview.slice(0, 120) : null,
      });
    }
    return entries;
  }
