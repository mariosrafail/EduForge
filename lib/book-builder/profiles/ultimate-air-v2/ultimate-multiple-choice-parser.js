...
export function parseSentenceMultipleChoice({ xml, activityCandidateId, sourceRelativePath, sourceSha256, nestedIdAllocator = null }) {
  ...
  const questionIds = allocateNestedCandidateIds("question", activityCandidateId, sentences, (sentence) => sentence?.["@_id"], "sentence", {
    idAllocator: nestedIdAllocator,
    sourceRelativePath,
    structuralPosition: (index) => `sentence[${index + 1}]`,
  });
  sentences.forEach((sentence, sentenceIndex) => {
    const questionId = questionIds[sentenceIndex];
    const sourceOptions = toArray(sentence.choice);
    const optionIds = allocateNestedCandidateIds("option", questionId, sourceOptions, (choice) => choice?.["@_id"], "choice", {
      idAllocator: nestedIdAllocator,
      sourceRelativePath,
      structuralPosition: (index) => `sentence[${sentenceIndex + 1}]/choice[${index + 1}]`,
    });
    const options = sourceOptions.map((choice, optionIndex) => ({ id: optionIds[optionIndex], publisherOptionId: choice?.["@_id"] === undefined ? null : String(choice["@_id"]), text: plainText(choice), textAvailability: plainText(choice) ? "structured" : "raster-only-or-missing", order: optionIndex + 1, geometry: geometry(choice) }));
