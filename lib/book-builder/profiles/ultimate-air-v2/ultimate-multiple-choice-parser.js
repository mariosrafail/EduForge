...
export function parseSentenceMultipleChoice({ xml, activityCandidateId, sourceRelativePath, sourceSha256 }) {
  ...
  sentences.forEach((sentence, sentenceIndex) => {
    ...
    if (options.some((item) => item.textAvailability !== "structured")) issues.push({ reasonCode: "raster_option_text_missing", questionId, sourceRelativePath, evidence: { rasterOptionCount: options.filter((item) => item.textAvailability !== "structured").length } });
  });
