...
export async function buildPageCandidates({ sourceRoot, inventoryEntries, internalDocuments, concurrency = 8 }) {
  ...
  async function worker() {
    while (true) {
      ...
      assets[index] = { sourceRelativePath: entry.path, component: match[1], unit: Number(match[2]), part: Number(match[4]), quality: match[3]?.toUpperCase() || "SPECIAL", width: dimensions.width, height: dimensions.height, byteSize: source.stat.size, sha256: entry.sha256 || await sha256File(source.absolutePath) };
    }
  }
  await Promise.all(Array.from({ length: Math.min(Math.max(1, concurrency), Math.max(1, pageEntries.length)) }, () => worker()));
  const groups = new Map();
  for (const asset of assets) { const key = `${asset.component}\0${asset.unit}\0${asset.part}`; if (!groups.has(key)) groups.set(key, { component: asset.component, unit: asset.unit, part: asset.part, variants: [] }); groups.get(key).variants.push(asset); }
  const reviewItems = [];
  const spreads = [...groups.values()].map((spread) => {
    spread.variants.sort((a, b) => a.quality.localeCompare(b.quality));
    const unitParamsPath = [...internalDocuments.keys()].find((sourcePath) => sourcePath.toLowerCase().endsWith(`/book1/${spread.component.toLowerCase()}/${spread.unit}/unit_params.iwb`));
  ...
  }).sort((a, b) => a.component.localeCompare(b.component) || a.unit - b.unit || a.part - b.part);
