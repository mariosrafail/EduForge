...
export function parseDragAndDrop({ xml, activityCandidateId, sourceRelativePath, sourceSha256, nestedIdAllocator = null }) {
  ...
  const draggableIds = allocateNestedCandidateIds("drag", activityCandidateId, sourceDrags, (drag) => drag?.["@_id"], "drag", {
    idAllocator: nestedIdAllocator,
    sourceRelativePath,
    structuralPosition: (index) => `drag[${index + 1}]`,
  });
  const targetIds = allocateNestedCandidateIds("target", activityCandidateId, sourceDrops, (drop) => drop?.["@_id"], "drop", {
    idAllocator: nestedIdAllocator,
    sourceRelativePath,
    structuralPosition: (index) => `drop[${index + 1}]`,
  });
  const draggables = sourceDrags.map((drag, index) => ({ id: draggableIds[index], publisherId: drag?.["@_id"] === undefined ? null : String(drag["@_id"]), label: plainText(drag), labelAvailability: plainText(drag) ? "structured" : "raster-only-or-missing", geometry: geometry(drag), sourceEvidence: [{ sourceRelativePath, sourceSha256 }] }));
  const targets = sourceDrops.map((drop, index) => ({ id: targetIds[index], publisherId: drop?.["@_id"] === undefined ? null : String(drop["@_id"]), label: plainText(drop), labelAvailability: plainText(drop) ? "structured" : "raster-only-or-missing", geometry: geometry(drop), sourceEvidence: [{ sourceRelativePath, sourceSha256 }] }));
