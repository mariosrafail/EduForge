...
export function parseDragAndDrop({ xml, activityCandidateId, sourceRelativePath, sourceSha256 }) {
  ...
  const draggables = sourceDrags.map((drag, index) => ({ id: nestedCandidateId("drag", activityCandidateId, drag?.["@_id"] ?? `drag-${index + 1}`), publisherId: drag?.["@_id"] === undefined ? null : String(drag["@_id"]), label: plainText(drag), labelAvailability: plainText(drag) ? "structured" : "raster-only-or-missing", geometry: geometry(drag) }));
  const targets = sourceDrops.map((drop, index) => ({ id: nestedCandidateId("target", activityCandidateId, drop?.["@_id"] ?? `drop-${index + 1}`), publisherId: drop?.["@_id"] === undefined ? null : String(drop["@_id"]), label: plainText(drop), labelAvailability: plainText(drop) ? "structured" : "raster-only-or-missing", geometry: geometry(drop) }));
  ...
  if (draggables.some((item) => item.labelAvailability !== "structured")) issues.push({ reasonCode: "raster_drag_label_missing", sourceRelativePath, evidence: { rasterDragLabelCount: draggables.filter((item) => item.labelAvailability !== "structured").length } });
