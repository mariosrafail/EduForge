...
export async function buildMenuAndBranding({ sourceRoot, inventoryEntries, internalDocuments }) {
  ...
  const sourceBookRoot = menuPath.match(/\/assets\/books\/(book\d+)\//i)?.[1] || "book1";
  const menuButtons = elementsNamed(internalDocuments.get(menuPath), "menuButton").map((item, index) => {
    ...
    return { id: stableId("menu", `${menuPath}\0${attrs.name || index}\0${attrs.url || ""}`), sourceRelativePath: menuPath, sourceBookRoot, name: attrs.name || null, url: attrs.url || null, active: booleanValue(attrs.active), visible: booleanValue(attrs.visible), x: finiteNumber(attrs.x), y: finiteNumber(attrs.y), width: finiteNumber(attrs.width), height: finiteNumber(attrs.height), scale: finiteNumber(attrs.scale), scaleOnHover: finiteNumber(attrs.scaleOnOver), scaleOnDown: finiteNumber(attrs.scaleOnDown), textureTriple, proposedDestination: destination, confidence: textureTriple.length === 3 && destination.kind !== "unresolved" ? 1 : 0.5 };
  });
  const reviewItems = [];
  ...
  return {
    menu: { schemaVersion: "1.0", parserId: "ultimate-air-v2-menu", parserVersion: "1.0", sourceRelativePath: menuPath, sourceBookRoot, summary: { buttonCount: menuButtons.length }, buttons: menuButtons },
