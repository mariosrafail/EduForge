...
export function parseQuestionBank({ xml, activityCandidateId, sourceRelativePath, sourceSha256, nestedIdAllocator = null }) {
  ...
  const questionIds = allocateNestedCandidateIds("question", activityCandidateId, sourceQuestions, (question) => question?.["@_id"], "question", {
    ...
    sourceRelativePath,
    structuralPosition: (index) => `question[${index + 1}]`,
  });
  sourceQuestions.forEach((question, questionIndex) => {
    ...
    const optionIds = allocateNestedCandidateIds("option", questionId, sourceOptions, (answer) => answer?.["@_id"], "option", {
      idAllocator: nestedIdAllocator,
      sourceRelativePath,
      structuralPosition: (index) => `question[${questionIndex + 1}]/answer[${index + 1}]`,
    });
