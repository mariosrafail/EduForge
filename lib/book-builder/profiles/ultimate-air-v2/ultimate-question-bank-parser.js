...
export function parseQuestionBank({ xml, activityCandidateId, sourceRelativePath, sourceSha256 }) {
  ...
  sourceQuestions.forEach((question, questionIndex) => {
    ...
    const options = toArray(question?.answer).map((answer, optionIndex) => ({
      ...
      publisherOptionId: answer?.["@_id"] === undefined ? null : String(answer["@_id"]), text: plainText(answer), order: optionIndex + 1,
    }));
    ...
    questions.push({ id: questionId, publisherQuestionId, prompt: plainText(question), responseKind: "single-option", options, sourceEvidence: [{ sourceRelativePath, sourceSha256 }] });
