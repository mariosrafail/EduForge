...
export async function buildActivitySignatures({ sourceRoot, inventoryEntries, iwbIndex, internalDocuments, pageCandidates, hotspotCandidates, mediaCandidates, concurrency = 8 }) {
  ...
  async function worker() {
    while (true) {
      ...
      const part = hotspotCandidates.parts.find((item) => (item.sourceBookRoot || "book1") === identity.bookRoot && item.component === identity.component && item.unit === identity.unit && item.part === identity.part);
      ...
      const page = pageCandidates.spreads.find((item) => (item.sourceBookRoot || "book1") === identity.bookRoot && item.component === identity.component && item.unit === identity.unit && item.part === identity.part);
      records[index] = { schemaVersion: "1.0", activityCandidateId: activityId, sourceObjectLocator, sourceBookRoot: identity.bookRoot, componentSourceDirectory: identity.component, componentKey: page?.componentKey || part?.componentKey || null, unit: identity.unit, unitKey: page?.unitKey || part?.unitKey || null, part: identity.part, object: identity.object,
        ...
        pageCandidateId: page ? `${page.component}/${page.unit}/part${page.part}` : null, pageKey: page?.pageKey || null, hotspotCandidateIds, mediaCandidateIds,
