...
export function buildUltimateStructure({ inventoryEntries, internalDocuments, pageCandidates = null }) {
  ...
  for (const entry of inventoryEntries) {
    ...
    if (!components.has(identity.component)) components.set(identity.component, { name: identity.component, sourceRelativePath: `Contents/Resources/assets/books/${identity.bookRoot}/${identity.component}`, units: new Map(), files: 0, bytes: 0 });
    const component = components.get(identity.component); component.files += 1; component.bytes += entry.byteSize;
    if (!component.units.has(identity.unit)) component.units.set(identity.unit, { number: identity.unit, parts: new Map(), sourceRelativePath: `${component.sourceRelativePath}/${identity.unit}`, navigation: [] });
  ...
  for (const [sourcePath, xml] of internalDocuments) {
    ...
    const unit = identity && components.get(identity.component)?.units.get(identity.unit);
    if (!unit) continue;
  ...
  for (const spread of pageCandidates?.spreads || []) pagesByComponent.set(spread.component, (pagesByComponent.get(spread.component) || 0) + 1);
  const normalized = [...components.values()].filter((item) => [...item.units.values()].some((unit) => unit.parts.size)).map((component) => {
    const units = [...component.units.values()].filter((unit) => unit.parts.size).sort((a, b) => a.number - b.number).map((unit) => ({
      number: unit.number, sourceRelativePath: unit.sourceRelativePath, navigation: unit.navigation,
