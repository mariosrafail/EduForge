...
import { assertStudentActivityCandidates, createNestedCandidateIdAllocator, validateTeacherSolutionCandidates } from "./activity-candidate-contract.js";
...
function safeIssueReview(issue, locator) {
  const targetId = issue.questionId || issue.optionId || issue.draggableId || issue.targetId || issue.responseFieldId || null;
  const suggestedDecisionKind = issue.reasonCode === "raster_prompt_missing" && issue.questionId ? "question_prompt_text"
    : issue.reasonCode === "raster_option_text_missing" ? "option_display_text"
      : issue.reasonCode === "raster_drag_label_missing" ? "draggable_display_label"
        : issue.reasonCode === "raster_target_label_missing" ? "target_display_label"
          : issue.reasonCode === "raster_response_prompt_missing" ? "response_field_prompt_text" : "activity_extraction_disposition";
  return createReviewItem({ category: "activity", locator: targetId ? `${locator}/${targetId}` : locator, reasonCode: issue.reasonCode, explanation: `Activity extraction requires review: ${issue.reasonCode.replaceAll("_", " ")}.`, suggestedDecisionKind, targetId, activityCandidateId: issue.activityCandidateId, evidence: issue.evidence ? [issue.evidence] : [] });
}
...
export function buildActivityExtraction({ signatures, iwbIndex, internalDocuments }) {
  ...
  for (const signature of signatures.records) {
    ...
    for (const [sourceRelativePath, xml] of documents) {
      ...
      const common = { xml, activityCandidateId: signature.activityCandidateId, sourceRelativePath, sourceSha256, nestedIdAllocator };
    ...
    const activityReviews = localIssues.map((issue) => safeIssueReview({ ...issue, activityCandidateId: signature.activityCandidateId }, signature.sourceObjectLocator)); reviewItems.push(...activityReviews);
    const candidate = { schemaVersion: "1.0", activityCandidateId: signature.activityCandidateId, sourceObjectLocator: signature.sourceObjectLocator, sourceBookRoot: signature.sourceBookRoot || "book1", componentCandidateId: `component:${signature.componentSourceDirectory}`, componentKey: signature.componentKey || null, unit: signature.unit, unitKey: signature.unitKey || null, part: signature.part, object: signature.object,
      ...
      ...content, mediaCandidateIds: signature.mediaCandidateIds, hotspotCandidateIds: signature.hotspotCandidateIds, pageCandidateId: signature.pageCandidateId, pageKey: signature.pageKey || null,
