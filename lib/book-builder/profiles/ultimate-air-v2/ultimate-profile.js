...
import { buildUltimateHierarchy } from "./ultimate-hierarchy.js";
...
export async function runUltimateProfile({ resolution, inventory, repositoryRoot, concurrency = 8 }) {
  ...
  const hierarchy = buildUltimateHierarchy({ structure, pages: pages.artifact, hotspots: hotspots.artifact, activities: activitySignatures.signatures, reviews: reviewQueue, menu: menu.menu });
  ...
  const artifacts = {
    ...
    structureCandidates: structure, componentHierarchy: hierarchy, pageCandidates: pages.artifact, menuModel: menu.menu, brandingModel: menu.branding, gafModel: menu.gaf || { schemaVersion: "1.0", parserId: "ultimate-air-v2-gaf", parserVersion: "1.0", status: "not_detected" },
