...
import { buildActivityContentAnchorFacts } from "./activity-content-facts.js";
...
export function buildUltimateFacts({ mainSwfPath, keyDiscovery, iwbIndex, structure, hierarchy = null, pages, menu, branding, gaf, hotspots, media, reviewQueue, atlasInventory = null, cropPlan = null, activitySignatures = null, activityExtraction = null }) {
  ...
  for (const root of hierarchy?.sourceBookRoots || []) for (const component of root.components || []) {
    facts.push(fact("component_hierarchy_candidate", component.sourceRelativeLocator, {
      sourceBookRootId: root.sourceBookRootId,
      componentKey: component.componentKey,
      sourceComponentName: component.sourceComponentName,
      detectedRole: component.detectedRole,
      groupingKind: component.groupingKind,
      unitGroupCount: component.unitGroups.length,
      pageCount: component.pageCount,
      activityCount: component.activityCount,
      reviewCount: component.reviewCount,
    }, component.roleConfidence ?? 0.4, [{ parserId: hierarchy.parserId, parserVersion: hierarchy.parserVersion }]));
    for (const group of component.unitGroups) facts.push(fact("unit_group_hierarchy_candidate", group.sourceRelativeLocator, {
      sourceBookRootId: root.sourceBookRootId,
      componentKey: component.componentKey,
    ...
    }, 1, [{ parserId: hierarchy.parserId, parserVersion: hierarchy.parserVersion }]));
  }
  ...
  for (const signature of activitySignatures?.records || []) {
    facts.push(fact("activity_signature_candidate", signature.sourceObjectLocator, { activityCandidateId: signature.activityCandidateId, structuralSignatureHash: signature.structuralSignatureHash, contentEvidenceHash: signature.contentEvidenceHash, publisherExerciseTypes: signature.publisherExerciseTypes, schemaFingerprints: signature.schemaFingerprints, fileCount: signature.sourceFiles.length }, 1, signature.sourceFiles));
    facts.push(fact("activity_disposition_candidate", `${signature.sourceObjectLocator}/disposition`, { activityCandidateId: signature.activityCandidateId, disposition: signature.disposition.disposition, normalizedCandidateType: signature.disposition.normalizedCandidateType, runtimeSupportStatus: signature.disposition.runtimeSupportStatus, reviewRequired: signature.disposition.reviewRequired }, signature.disposition.confidence, [{ structuralSignatureHash: signature.structuralSignatureHash }]));
  }
  for (const evidence of activityExtraction?.evidenceArtifact?.records || []) {
    facts.push(fact("activity_content_candidate", `${evidence.sourceObjectLocator}/content`, { activityCandidateId: evidence.activityCandidateId, contentAvailability: evidence.contentAvailability, contentEvidenceHash: evidence.contentEvidenceHash }, 1, [{ contentEvidenceHash: evidence.contentEvidenceHash }]));
    facts.push(fact("activity_solution_evidence_summary", `${evidence.sourceObjectLocator}/solution-evidence`, { activityCandidateId: evidence.activityCandidateId, solutionEvidenceCounts: evidence.solutionEvidenceCounts }, 1, [{ contentEvidenceHash: evidence.contentEvidenceHash }]));
  }
  for (const activity of activityExtraction?.studentArtifact?.candidates || []) {
    facts.push(...buildActivityContentAnchorFacts(activity, (kind, locator, value) => fact(kind, locator, value, 1, [{ detectedContentDigest: value.detectedContentDigest, sourceReferenceDigest: value.sourceReferenceDigest }])));
  }
