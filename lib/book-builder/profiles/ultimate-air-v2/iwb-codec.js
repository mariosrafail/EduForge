...
const PLAUSIBLE_ROOT = /^\s*(?:<\?xml[^>]*>\s*)?<([A-Za-z_][\w:.-]*)\b/;
...
export function elementsNamed(xml, name) {
  ...
  return [...String(xml).matchAll(pattern)].map((match) => ({ source: match[0], attributes: parseAttributes(match[0]), offset: match.index }));
}
...
export function summarizeSafeXml(xml) {
  ...
  for (const match of structuralXml.matchAll(/<([A-Za-z_][\w:.-]*)\b([^<>]*)>/g)) {
    ...
    if (tag.toLowerCase() === "exercise" && attributes.type) exerciseTypes.add(attributes.type);
  }
...
export function decodeIwb(input, key) {
  ...
  const rootMatch = xml.match(PLAUSIBLE_ROOT);
  const root = rootMatch?.[1] || null;
