...
export function buildHotspotCandidates({ inventoryEntries, internalDocuments, structure, pageCandidates }) {
  ...
  for (const component of structure.components) for (const unit of component.units) for (const part of unit.parts) objectsByPart.set(`${component.name}\0${unit.number}\0${part.number}`, part.objectNumbers);
  const pageByPart = new Map(pageCandidates.spreads.map((spread) => [`${spread.component}\0${spread.unit}\0${spread.part}`, spread]));
  ...
  for (const [sourcePath, xml] of internalDocuments) {
    ...
    const key = `${identity.component}\0${identity.unit}\0${identity.part}`; const objectNumbers = objectsByPart.get(key) || [];
    ...
    parts.push({ sourceRelativePath: sourcePath, sourceSha256: sourceHashByPath.get(sourcePath) || null, component: identity.component, unit: identity.unit, part: identity.part, buttonCount: buttons.length, quadCount: quads.length, objectDirectoryCount: objectNumbers.length, exactCardinality, coordinateSpace: canonical ? { width: canonical.width, height: canonical.height, evidence: canonical.sourceRelativePath } : null, hotspots: candidates, quads: normalizedQuads });
  }
