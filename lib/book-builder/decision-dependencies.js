...
} from "./activity-content-overrides.js";
...
export function pageDecisionTargetId(spread) {
  return spread.sourceBookRoot && spread.sourceBookRoot !== "book1"
    ? `page_${digest(spread.sourceBookRoot, spread.component, spread.unit, spread.part)}`
    : `page_${digest(spread.component, spread.unit, spread.part)}`;
}

function targetTypeForKind(kind) {
  if (ACTIVITY_CONTENT_DECISION_KINDS.has(kind)) return activityContentDecisionDefinition(kind).targetType;
...
function factMatchesTarget(fact, target, kind) {
  if (ACTIVITY_CONTENT_DECISION_KINDS.has(kind)) {
    const definition = activityContentDecisionDefinition(kind);
    return fact.kind === definition.factKind
      && fact.value?.activityCandidateId === target.activity.activityCandidateId
      && fact.value?.targetId === (target.node.id || target.activity.activityCandidateId);
  }
  ...
  if (kind === "printed_page_number" || kind === "canonical_page_variant") {
    ...
    return ["part_page_candidate", "page_asset_variant"].includes(fact.kind)
      && locators.has(fact.sourceLocator);
  }
...
function relevantReviews(queue, target, kind, dependencyFactIds) {
  if (ACTIVITY_CONTENT_DECISION_KINDS.has(kind)) return list(queue?.items).filter((item) => item.suggestedDecisionKind === kind && item.targetId === (target.node.id || target.activity.activityCandidateId));
...
function resolveRawTarget(artifacts, kind, targetId) {
  if (ACTIVITY_CONTENT_DECISION_KINDS.has(kind)) return findActivityContentTarget(artifacts.activities, kind, targetId);
...
function allowedValuesForTarget(kind, target, artifacts) {
  if (ACTIVITY_CONTENT_DECISION_KINDS.has(kind)) return null;
...
function targetSummary(target, kind, targetId) {
  if (ACTIVITY_CONTENT_DECISION_KINDS.has(kind)) {
    ...
    return {
      ...
      activityCandidateId: target.activity.activityCandidateId,
      label: definition.label,
      field: definition.field,
      detectedValue: detectedActivityContentValue(target),
      availability: target.node[definition.availability] || (detectedActivityContentValue(target) ? "structured" : "raster-only-or-missing"),
      sourceRelativeLocator: target.activity.sourceObjectLocator,
    };
  }
...
export function resolveDecisionContext({ project, artifacts, kind, targetId, value }) {
  ...
  const reviewFactIds = ACTIVITY_CONTENT_DECISION_KINDS.has(kind) ? [] : list(project.detectedFacts)
