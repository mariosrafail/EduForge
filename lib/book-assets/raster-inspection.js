...
const RASTER_FORMATS = Object.freeze(["png", "jpeg", "webp"]);
...
function invalidRaster(reason = "invalid_raster") {
  throw Object.assign(new Error("invalid_raster"), { code: reason });
}

function normalizePolicy({ allowedFormats, maximumBytes, maximumDimension, maximumPixels }) {
  if (!Array.isArray(allowedFormats) || !allowedFormats.length || new Set(allowedFormats).size !== allowedFormats.length
    || allowedFormats.some((format) => !RASTER_FORMATS.includes(format))
    || !Number.isSafeInteger(maximumBytes) || maximumBytes < 1
    || !Number.isSafeInteger(maximumDimension) || maximumDimension < 1 || maximumDimension > 0xffffffff
    || !Number.isSafeInteger(maximumPixels) || maximumPixels < 1) {
    throw new TypeError("Invalid raster inspection policy.");
  }
  return Object.freeze({ allowedFormats: new Set(allowedFormats), maximumBytes, maximumDimension, maximumPixels });
}

function validateDimensions(width, height, policy) {
  if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width < 1 || height < 1) {
    invalidRaster("invalid_raster_dimensions");
  }
  if (width > policy.maximumDimension || height > policy.maximumDimension) invalidRaster("raster_dimension_limit");
  if (width > Math.floor(policy.maximumPixels / height)) invalidRaster("raster_pixel_limit");
}
...
async function inspectPng(bytes, policy) {
...
function inspectJpeg(bytes, policy) {
...
function inspectVp8(bytes, start, length, policy) {
...
function inspectVp8l(bytes, start, length, policy) {
...
function inspectAnimatedWebpFrame(bytes, chunk, canvasWidth, canvasHeight, policy) {
  ...
  const image = imageChunks[0].type === "VP8 "
    ? inspectVp8(bytes, imageChunks[0].dataOffset, imageChunks[0].length, policy)
    : inspectVp8l(bytes, imageChunks[0].dataOffset, imageChunks[0].length, policy);
  if (image.width !== width || image.height !== height) invalidRaster();
  return Boolean(alphaChunks.length || image.alpha);
}

function inspectWebp(bytes, policy) {
  ...
  if (first.type === "VP8 " || first.type === "VP8L") {
    ...
    const image = first.type === "VP8 " ? inspectVp8(bytes, first.dataOffset, first.length, policy) : inspectVp8l(bytes, first.dataOffset, first.length, policy);
  ...
  if (animated) {
    ...
    let hasAlpha = false;
    for (const frame of frames) hasAlpha = inspectAnimatedWebpFrame(bytes, frame, width, height, policy) || hasAlpha;
    ...
    const image = images[0].type === "VP8 "
      ? inspectVp8(bytes, images[0].dataOffset, images[0].length, policy)
      : inspectVp8l(bytes, images[0].dataOffset, images[0].length, policy);
    if (image.width !== width || image.height !== height || Boolean(flags & 0x10) !== Boolean(alphaChunks.length || image.alpha)) invalidRaster();
  }
  return { format: "webp", width, height };
}

async function inspectRasterStructure(bytes, policy) {
  if (bytes.length >= 8 && bytes.subarray(0, 8).equals(PNG_SIGNATURE)) {
    if (!policy.allowedFormats.has("png")) invalidRaster("raster_format_not_allowed");
    return inspectPng(bytes, policy);
  }
  if (bytes.length >= 2 && bytes[0] === 0xff && bytes[1] === 0xd8) {
    if (!policy.allowedFormats.has("jpeg")) invalidRaster("raster_format_not_allowed");
    return inspectJpeg(bytes, policy);
  }
  if (bytes.length >= 12 && bytes.toString("latin1", 0, 4) === "RIFF" && bytes.toString("latin1", 8, 12) === "WEBP") {
    if (!policy.allowedFormats.has("webp")) invalidRaster("raster_format_not_allowed");
    return inspectWebp(bytes, policy);
  }
  invalidRaster();
}

export async function inspectRasterBytes(input, options) {
  const policy = normalizePolicy(options || {});
  const bytes = Buffer.from(input || []);
  if (!bytes.length) invalidRaster("empty_raster");
  if (bytes.length > policy.maximumBytes) invalidRaster("raster_too_large");
  const metadata = await inspectRasterStructure(bytes, policy);
  return {
    bytes,
...
export async function inspectManagedRaster(input) {
  ...
  let metadata;
  try {
    metadata = await inspectRasterBytes(bytes, {
      allowedFormats: RASTER_FORMATS,
      maximumBytes: MANAGED_RASTER_MAXIMUM_BYTES,
      maximumDimension: MANAGED_RASTER_MAXIMUM_DIMENSION,
      maximumPixels: MANAGED_RASTER_MAXIMUM_DIMENSION ** 2,
    });
  } catch (error) {
