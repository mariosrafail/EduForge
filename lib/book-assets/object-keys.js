...
export function buildNativeActivityAssetObjectKey({ bookSlug, componentSlug, activityId, assetSlot = null, checksum, extension, purpose = "native-asset" }) {
  const digest = String(checksum || "").toLowerCase();
  ...
  if (purpose === "teacher-answer") {
    if (![".png", ".jpg", ".webp"].includes(suffix)) throw new Error("Teacher answer assets must be raster images.");
    segments.push("teacher-answers");
  }
...
export function buildUnitExtraAssetStagingKey({ bookSlug, componentSlug, unitSlug, itemId, uploadId }) {
  return validateObjectKey([
    ...
    validatedUuidSegment(uploadId, "upload id"),
    "staging",
    String(itemId || "").startsWith("audio-") ? "audio" : "video",
  ].join("/"));
}
...
export function buildUnitExtraAssetObjectKey({ bookSlug, componentSlug, unitSlug, itemId, checksum, extension }) {
  const digest = String(checksum || "").toLowerCase();
  const suffix = String(extension || "").toLowerCase();
  if (!/^[a-f0-9]{64}$/.test(digest) || ![".mp3", ".mp4"].includes(suffix)) throw new Error("Unit Extra media identity is invalid");
  return validateObjectKey([
    "builder-unit-extra-assets",
    normalizeObjectKeySegment(bookSlug, "book slug"),
    normalizeObjectKeySegment(componentSlug, "component slug"),
...
export function buildBookAssetHostedTeacherUiPublicKey({
