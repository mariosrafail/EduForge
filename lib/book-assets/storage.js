...
function safeProviderCode(error) {
  for (const value of [error?.name, error?.Code, error?.code]) {
    ...
    if (normalized !== "Error" && /^[A-Za-z0-9_.-]{1,64}$/.test(normalized)) return normalized;
  }
  return undefined;
}
...
export function classifyImmutableCopyProviderFailure(error) {
  ...
  else if (providerStatus === undefined && COPY_TRANSPORT_CODES.has(providerCode)) failureClass = "copy_transport_failed";
