...
import { TEACHER_ANDROID_APPLICATION_ID, TEACHER_ANDROID_APPLICATION_LABEL } from "./android-contract.js";
...
export async function verifyTeacherProjectApkArchive({ apkPath, distRoot, project, stagingManifest, inspectApk }) {
  ...
  assert.equal(manifest.applicationId, TEACHER_ANDROID_APPLICATION_ID);
  assert.equal(manifest.applicationLabel, TEACHER_ANDROID_APPLICATION_LABEL);
