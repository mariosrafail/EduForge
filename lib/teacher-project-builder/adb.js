...
const APPLICATION_ID = "com.eduforge.offlinebooks";
const MAIN_ACTIVITY = `${APPLICATION_ID}/.MainActivity`;
...
export async function installAndLaunchTeacherApk({ repositoryRoot, apkPath, serial, environment, runProcess = runFixedProcess } = {}) {
  ...
  try {
    ...
    await runProcess(adb.command, ["-s", serial, "shell", "am", "start", "-n", MAIN_ACTIVITY], { cwd: repositoryRoot, env: environment || process.env });
  } catch {
    throw new TeacherProjectError("android_install_or_launch_failed", 502);
  }
  return { status: "launched", serial, applicationId: APPLICATION_ID, activity: MAIN_ACTIVITY, replacedExistingDebugApp: true };
}

export { APPLICATION_ID as TEACHER_ANDROID_APPLICATION_ID, MAIN_ACTIVITY as TEACHER_ANDROID_MAIN_ACTIVITY };
