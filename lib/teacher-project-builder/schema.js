...
import {
  ...
  TEACHER_PROJECT_LEGACY_SCHEMA_VERSION,
...
import {
  createBlankTeacherContent,
  teacherProjectContentAssetIds,
  teacherProjectContentStatus,
  validateTeacherProjectContent,
...
function assertAssetReferenceTypes(project) {
  ...
  teacherProjectContentAssetIds(project.content).forEach((id) => expectType(id, IMAGE_TYPES, "invalid_teacher_page_image_reference"));
}

function validateCommonProject(candidate) {
...
function validateVersionOneTeacherProject(candidate) {
  ...
  if (candidate.schemaVersion !== TEACHER_PROJECT_LEGACY_SCHEMA_VERSION || candidate.kind !== TEACHER_PROJECT_KIND) fail("unsupported_teacher_project_schema");
  const legacy = structuredClone(candidate);
  validateCommonProject(legacy);
  for (const assetId of referencedAssetIds(legacy.shell)) if (!legacy.assets[assetId]) fail("missing_teacher_asset_reference", { assetId });
  const normalized = { ...legacy, schemaVersion: TEACHER_PROJECT_SCHEMA_VERSION, content: createBlankTeacherContent() };
  return validateCurrentTeacherProject(normalized);
}

function validateCurrentTeacherProject(candidate) {
  exactObject(candidate, ["schemaVersion", "kind", "projectId", "displayName", "revision", "savedAt", "shell", "content", "assets", "build"], "invalid_teacher_project");
  ...
  validateCommonProject(candidate);
  validateTeacherProjectContent(candidate.content);
  for (const assetId of [...referencedAssetIds(candidate.shell), ...teacherProjectContentAssetIds(candidate.content)]) {
    if (!candidate.assets[assetId]) fail("missing_teacher_asset_reference", { assetId });
  }
...
export function validateTeacherProject(candidate) {
  if (candidate?.schemaVersion === TEACHER_PROJECT_LEGACY_SCHEMA_VERSION) return validateVersionOneTeacherProject(candidate);
  return validateCurrentTeacherProject(candidate);
}
...
export function createBlankTeacherProject({ projectId, displayName, now = new Date().toISOString() }) {
  ...
  const project = {
    ...
    content: createBlankTeacherContent(),
    assets: {},
...
export function validateTeacherProjectDraft(candidate, existing) {
  exactObject(candidate, ["displayName", "expectedRevision", "shell", "content"], "invalid_teacher_project_save");
...
export function teacherProjectCompleteness(project) {
  project = validateTeacherProject(project);
  const missing = [];
  ...
  const contentStatus = teacherProjectContentStatus(project.content);
  return {
    complete: missing.length === 0 && contentStatus.valid,
    shellComplete: missing.length === 0,
    missing,
    requiredCount,
...
export function teacherProjectSummary(project) {
  ...
  return {
    ...
    completeness,
    contentStatus: completeness.contentStatus,
  };
}

export function teacherProjectReferencedAssetIds(project) {
  project = validateTeacherProject(project);
  return [...new Set([...referencedAssetIds(project.shell), ...teacherProjectContentAssetIds(project.content)])].sort();
}
