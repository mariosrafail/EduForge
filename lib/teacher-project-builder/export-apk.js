...
const capacitorWebRoot = path.join(repositoryRoot, "dist");
const trackedCapacitorGeneratedFiles = [
  path.join(repositoryRoot, "android", "app", "capacitor.build.gradle"),
  path.join(repositoryRoot, "android", "capacitor.settings.gradle"),
];
...
async function materializeCapacitorWebRoot(verifiedDistRoot) {
  if (path.dirname(capacitorWebRoot) !== repositoryRoot || path.basename(capacitorWebRoot) !== "dist") throw new TeacherProjectError("teacher_capacitor_web_path_unsafe", 500);
  const sourceInfo = await fs.lstat(verifiedDistRoot).catch(() => null);
  if (!sourceInfo?.isDirectory() || sourceInfo.isSymbolicLink()) throw new TeacherProjectError("teacher_verified_dist_unavailable", 500);
  const destinationInfo = await fs.lstat(capacitorWebRoot).catch(() => null);
  if (destinationInfo?.isSymbolicLink()) throw new TeacherProjectError("teacher_capacitor_web_path_unsafe", 500);
  if (destinationInfo) await fs.rm(capacitorWebRoot, { recursive: true, force: true });
  await fs.cp(verifiedDistRoot, capacitorWebRoot, { recursive: true, errorOnExist: true, force: false });
}

async function preservingTrackedCapacitorFiles(operation) {
  const snapshots = await Promise.all(trackedCapacitorGeneratedFiles.map(async (file) => ({ file, bytes: await fs.readFile(file) })));
  try { return await operation(); }
  finally {
    await Promise.all(snapshots.map(({ file, bytes }) => fs.writeFile(file, bytes)));
  }
}
...
export async function exportTeacherProjectApk({
  ...
  return queueAndroidBuild(async () => {
    ...
    return preservingTrackedCapacitorFiles(async () => {
      const env = { ...process.env, CAPACITOR_BUILD_MODE: "teacher" };
      onStage("Syncing Android");
      await materializeCapacitorWebRoot(prepared.distRoot);
