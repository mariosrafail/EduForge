...
export class TeacherProjectJobManager {
  ...
  async validateRevision(projectId, expectedRevision) {
    ...
    if (!status.completeness.complete) throw new TeacherProjectError("incomplete_teacher_project", 409, {
      missing: status.completeness.missing,
      contentIssues: status.completeness.contentStatus.issuesByUnit,
    });
    return status;
  }
