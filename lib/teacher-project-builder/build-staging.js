...
import { teacherProjectCompleteness, teacherProjectReferencedAssetIds } from "./schema.js";
...
async function validateGafBundle(project, projectDirectory) {
  ...
  const summary = parseGafSummary(await fs.readFile(gafPath));
  ...
  if (!requiredCount || sdAtlases.length < requiredCount || hdAtlases.length < requiredCount) {
    throw new TeacherProjectError("incomplete_teacher_gaf_bundle", 409, { requiredSdAtlases: requiredCount, requiredHdAtlases: requiredCount });
  }
  return { version: summary.version, frameCount: summary.timeline.frames, requiredSdAtlases: requiredCount, requiredHdAtlases: requiredCount };
}
...
export async function prepareTeacherProjectBuild({ store, projectId }) {
  ...
  if (!completeness.complete) throw new TeacherProjectError("incomplete_teacher_project", 409, { missing: completeness.missing });
  ...
  const manifest = {
    ...
    assetIds,
    gaf,
    applicationId: "com.eduforge.offlinebooks",
  };
...
export async function validatePreparedTeacherProjectBuild(prepared) {
  const completeness = teacherProjectCompleteness(prepared.project);
  if (!completeness.complete) throw new TeacherProjectError("incomplete_teacher_project", 409);
