-- Preserve the canonical Students Book Unit Extras authoring document while a
-- page is tombstoned. Its page visibility mapping is dormant whenever the page
-- is absent from the active-page projection, and its managed assets are Unit-owned.
...
begin
  ...
  if exists(select 1 from book_media_assets where package_slug=requested_book_slug and component_slug=requested_component_slug and page_id=page_slug)
    or exists(select 1 from builder_component_documents where book_component_id=scope.book_component_id
      ...
      and not (
        requested_book_slug='ultimate-b2' and requested_component_slug='ultimate-b2-students-book'
        and document_type='unit_extras' and document_key='default'
      )
