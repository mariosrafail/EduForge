...
import { canEditOwnedContent } from "./_resource-access.js";
  ...
  try {
    ...
    const existing = await sql`
      select l.id, l.course_id, l.title, l.subtitle, l.instructions, l.status,
             coalesce(l.school_id, c.school_id) as school_id, l.ownership_type, l.created_by
    ...
    if (!canEditOwnedContent(auth.currentUser, lesson)) return json(403, { error: "Forbidden" });
    ...
    await sql`
      ...
      where id = ${id}
        ...
        and (${auth.currentUser.role === "admin"} or (ownership_type = 'custom' and created_by = ${auth.currentUser.id}))
    `;
