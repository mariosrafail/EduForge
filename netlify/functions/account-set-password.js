...
export async function handler(event) {
  ...
  try {
    ...
    const lookup = await sql`select u.email,u.password_hash from account_tokens t join app_users u on u.id=t.user_id where t.token_hash=${hashToken(rawToken)} and t.purpose='initial_password' and t.used_at is null and t.revoked_at is null and t.expires_at > now() and u.status='invited' limit 1`;
    ...
    const rows = await consumePasswordToken(sql, { rawToken, purpose: "initial_password", passwordHash: await hashPassword(password), session, fingerprint });
  ...
  } catch (error) { return schemaFailureResponse(error) || safeServerError(error, "Password could not be set"); }
}
