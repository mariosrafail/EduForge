...
export async function handler(event) {
  ...
  try {
    ...
    if (await checkRateLimit(sql, { scope: "forgot_password", fingerprint, emailHash, limit: 5 })) return json(429, { message: genericForgotPasswordMessage });
    ...
    if (user) {
      ...
      `;
      delivery = await deliverAccountEmail({ recipient: user.email, templateType: "password_reset", rawToken, outboxId, name: user.full_name });
