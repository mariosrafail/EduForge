...
function splitLongToken(token, width, fontSize) {
  ...
  for (const character of token) {
    if (current && nativeAnswerTextWidth(current + character, fontSize) > width) { chunks.push(current); current = character; }
...
function wrap(text, width, fontSize) {
  ...
  for (const token of text.split(" ")) {
    ...
    if (nativeAnswerTextWidth(candidate, fontSize) <= width) { current = candidate; continue; }
    if (current) { lines.push(current); current = ""; }
    if (nativeAnswerTextWidth(token, fontSize) <= width) { current = token; continue; }
    const chunks = splitLongToken(token, width, fontSize);
...
export function autoFitNativeOpenResponseAnswer({ text, responseRegion }) {
  ...
  for (let fontSize = Math.floor(presentation.answerFontSizeMax); fontSize >= Math.ceil(presentation.answerFontSizeMin); fontSize -= 1) {
    const lines = wrap(normalizedText, availableWidth, fontSize);
    if (lines.length <= presentation.lineCount && lines.every((line) => nativeAnswerTextWidth(line, fontSize) <= availableWidth)) {
      return { fontSize, lines, baselines: baselines.slice(0, lines.length), fits: true, overflowReason: null };
    }
  }
  const lines = wrap(normalizedText, availableWidth, presentation.answerFontSizeMin);
