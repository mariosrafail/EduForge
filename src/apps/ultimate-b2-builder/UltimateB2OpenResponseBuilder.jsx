...
export function UltimateB2OpenResponseBuilder() {
  ...
  const responseRegions = useMemo(() => payload ? payload.publicAuthoring.questions.flatMap((question, index) => question.responseRegion?.area ? [{ id: question.id, label: `Question ${index + 1} response region`, ...question.responseRegion.area }] : []) : [], [payload]);
  ...
  const updateResponseRegions = (areas) => change((next) => {
    next.publicAuthoring.questions.forEach((question) => {
      ...
      if (latest) question.responseRegion.area = { left: latest.left, top: latest.top, width: latest.width, height: latest.height };
      ...
      {section === "Response Regions" && <div className="open-response-region-workspace">
        ...
        <aside className="open-response-region-properties">
          ...
          {payload.publicAuthoring.questions.map((question, index) => question.id === selectedQuestionId && <div className="open-response-selected-region" key={question.id}>
            ...
            {question.responseRegion.area ? <dl><div><dt>Left</dt><dd>{question.responseRegion.area.left.toFixed(2)}%</dd></div><div><dt>Top</dt><dd>{question.responseRegion.area.top.toFixed(2)}%</dd></div><div><dt>Width</dt><dd>{question.responseRegion.area.width.toFixed(2)}%</dd></div><div><dt>Height</dt><dd>{question.responseRegion.area.height.toFixed(2)}%</dd></div></dl> : <p className="response-region-warning">Draw a replacement region before saving.</p>}
            ...
            {question.responseRegion.area && payload.teacherAuthoring.modelAnswers[index].text.length > question.responseRegion.area.width * question.responseRegion.area.height * .18 && <p className="response-region-warning">The current text may need more region height or a smaller font scale to remain comfortably readable.</p>}
