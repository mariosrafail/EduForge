...
const sections = Object.freeze([
  ...
  ["navigation", "Navigation / Window Controls"], ["units", "Units"], ["editions", "Editions"],
...
export function UltimateB2TeacherAppBuilder({ onSaved }) {
  ...
  return (
    <main className="b2-teacher-app-builder">
      <header className="b2-teacher-app-header"><div><span>Ultimate B2 bound authoring</span><h1>UI Controller / Book Setup</h1><p>Repository-backed shell and the same 22 page identities used by hotspots, activities and the Teacher pack.</p></div><div className="b2-teacher-save-state" role="status"><strong>{status}</strong>{error && <small>{error}</small>}<button type="button" onClick={save} disabled={!dirty || status === "Saving…"}><Save size={17} /> Save</button></div></header>
      ...
      <div className="b2-teacher-layout">
        <section className="b2-teacher-editor-panel">
          ...
          {section === "navigation" && <div className="b2-teacher-assets"><h2>Navigation / window controls</h2>{Object.entries(model.shell.navigation).map(([id, binding]) => slot(binding, id.replace(/([A-Z])/g, " $1").replace(/^./, (value) => value.toUpperCase())))}</div>}
          ...
          {section === "editions" && <div className="b2-teacher-assets"><h2>Book editions</h2>{assetPair(model.shell.editions, revision, draftUrls, importAsset, warnings)}<p>GB/WB bottom navigation remains textual and semantic; it is not converted into image controls.</p></div>}
