...
export const ultimateB2StudentsBookReviewValidationPolicy = Object.freeze({
...
export async function validateReviewContentPack(pack, policy = ultimateB2StudentsBookReviewValidationPolicy) {
  try {
    ...
    if (manifest?.schemaVersion !== 1 || manifest.minimumSupportedContentSchemaVersion !== 1 || manifest.minimumSupportedAppVersion !== "0.1.0") return { valid: false, reason: "Unsupported content schema." };
    if (manifest.packageId !== policy.packageId || manifest.componentId !== policy.componentId) return { valid: false, reason: "Unexpected content package identity." };

    const records = { "catalog.json": catalog, "activities.json": activities, "assets-manifest.json": assetsManifest };
    for (const [name, value] of Object.entries(records)) {
      const expected = manifest.files?.[name]?.semanticSha256;
      if (!expected || await sha256(stableStringify(value)) !== expected) return { valid: false, reason: `Integrity check failed for ${name}.` };
    }
    ...
    if (assetsManifest?.assets?.length !== manifest.assetCount || new Set(assetsManifest.assets.map((asset) => asset.logicalKey)).size !== manifest.assetCount) return { valid: false, reason: "Asset manifest is incomplete." };
