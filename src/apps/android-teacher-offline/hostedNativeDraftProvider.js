...
export async function loadHostedNativeDraftTeacherActivity(publicEntry, { context = resolveHostedViewerRuntimeContext(), fetchImpl = fetch, signal } = {}) {
  if (context.kind !== HOSTED_VIEWER_RUNTIME_MODES.BUILDER_PREVIEW || !["open-response", "single-choice", "complete-sentences"].includes(publicEntry?.kind)) return null;
...
export function shouldLoadHostedNativeDraftTeacherActivity(publicEntry, teacherMode) {
  return Boolean(teacherMode && ["open-response", "single-choice", "complete-sentences"].includes(publicEntry?.kind));
}
