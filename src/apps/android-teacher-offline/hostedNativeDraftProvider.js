...
export async function loadHostedNativeDraftTeacherActivity(publicEntry, { context = resolveHostedViewerRuntimeContext(), identity = currentIdentity(), fetchImpl = globalThis.fetch, signal } = {}) {
  if (context.kind !== HOSTED_VIEWER_RUNTIME_MODES.BUILDER_PREVIEW || !["open-response", "single-choice", "complete-sentences", "listening", "oldschool-listening", "drag-drop"].includes(publicEntry?.kind)) return null;
...
export function shouldLoadHostedNativeDraftTeacherActivity(publicEntry, teacherMode) {
  return Boolean(teacherMode && ["open-response", "single-choice", "complete-sentences", "listening", "oldschool-listening", "drag-drop"].includes(publicEntry?.kind));
}
