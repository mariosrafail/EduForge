...
export function hostedReleasePath(runtimeContext, identity, suffix) {
  if (runtimeContext?.kind !== HOSTED_VIEWER_RUNTIME_MODES.RELEASE_PREVIEW || !SAFE_ID.test(String(identity?.bookSlug || "")) || !SAFE_ID.test(String(identity?.componentSlug || ""))
    || !UUID.test(String(runtimeContext.releaseId || "")) || !/^(?:public|teacher-ui|teacher-solution\/[a-z0-9][a-z0-9-]{0,127}|native-teacher\/[a-z0-9][a-z0-9-]{0,127}|assets\/[a-f0-9]{64}\.(?:png|jpg|webp|mp3|mp4|pdf))$/.test(suffix)) throw new Error("Invalid hosted release preview path.");
