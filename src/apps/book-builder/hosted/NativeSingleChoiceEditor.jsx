...
export function NativeSingleChoiceEditor({ bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {}, onSaved = () => {} }) {
  ...
  return <section className="native-activity-foundation native-single-choice-editor">
    <header><div><span>Native draft · {readiness.ready ? "Content complete" : "Content incomplete"}</span><h2>{publicDraft.metadata.title}</h2><p>{placementLabel} · Multiple Choice</p></div><details className="builder-technical-details"><summary>Technical details</summary><dl><div><dt>Stable ID</dt><dd><code>{activityId}</code></dd></div><div><dt>Revisions</dt><dd>Public {state.publicRevision} · Teacher {state.teacherRevision}</dd></div></dl></details></header>
