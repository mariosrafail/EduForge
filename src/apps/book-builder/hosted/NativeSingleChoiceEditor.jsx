...
export function NativeSingleChoiceEditor({ bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {}, onSaved = () => {} }) {
  ...
  return <section className="native-activity-foundation native-single-choice-editor studio-editor">
    ...
    {["content", "visual", "answer-key", "preview"].includes(mode) ? <div className="native-single-choice-back" data-authoring-mode={mode}>
      ...
      {selected ? <section className="native-or-question-editor"><header><strong>Question {questions.indexOf(selected) + 1}</strong><code>{selected.id}</code><div><button type="button" disabled={questions.indexOf(selected) === 0} onClick={() => moveQuestion(-1)}>Move Up</button><button type="button" disabled={questions.indexOf(selected) === questions.length - 1} onClick={() => moveQuestion(1)}>Move Down</button><button type="button" onClick={() => deleteQuestion(selected.id)}>Delete Question</button></div></header><StudioField label="Prompt"><textarea readOnly={mode === "answer-key"} maxLength={NATIVE_SINGLE_CHOICE_LIMITS.promptLength} value={selected.prompt} onChange={(event) => mutatePublic((next) => { next.parts[0].interaction.questions.find((question) => question.id === selected.id).prompt = event.target.value; })} /></StudioField><fieldset><legend>{mode === "answer-key" ? "Private correct answer" : "Options"} {answer ? null : <em>· Needs answer</em>}</legend>{selected.options.map((option, index) => mode === "answer-key" ? <label className="native-single-choice-answer-option" key={option.id}><input type="radio" name={`correct-${selected.id}`} aria-label={`Option ${index + 1}: ${option.text || "Untitled option"}`} checked={answer?.correctOptionId === option.id} onChange={() => setAnswer(option.id)} /><span><strong>Option {index + 1}</strong><span>{option.text || "Untitled option"}</span><small>{answer?.correctOptionId === option.id ? "Selected correct answer" : "Select as correct"}</small></span></label> : <div className="native-single-choice-option-editor" key={option.id}><span aria-hidden="true">{index + 1}</span><input type="text" aria-label={`Option ${index + 1}`} maxLength={NATIVE_SINGLE_CHOICE_LIMITS.optionTextLength} value={option.text} onChange={(event) => mutatePublic((next) => { next.parts[0].interaction.questions.find((question) => question.id === selected.id).options.find((item) => item.id === option.id).text = event.target.value; })} /><button type="button" aria-label={`Move option ${index + 1} up`} disabled={index === 0} onClick={() => moveOption(option.id, -1)}>↑</button><button type="button" aria-label={`Move option ${index + 1} down`} disabled={index === selected.options.length - 1} onClick={() => moveOption(option.id, 1)}>↓</button><button type="button" disabled={selected.options.length <= 2} onClick={() => deleteOption(option.id)}>Delete</button><code>{option.id}</code></div>)}</fieldset><StudioButton disabled={selected.options.length >= NATIVE_SINGLE_CHOICE_LIMITS.optionsMaximum} onClick={addOption}><Plus aria-hidden="true" />Add Option</StudioButton></section> : <p>No questions yet. Add a question to begin.</p>}</div>
      : null}
