...
export function NativeCompleteSentencesEditor({ bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {}, onSaved = () => {} }) {
  ...
  useEffect(() => {
    ...
    setTab("content"); setDirty(false); onDirtyChange(false);
    Promise.all([
  ...
  const removeItem = () => {
    ...
    setSelectedItemId(items[index + 1]?.id || items[index - 1]?.id || null); setSelectedHotspotId(null);
  };
  const moveItem = (offset) => mutatePair((nextPublic, nextTeacher) => {
    const list = nextPublic.parts[0].interaction.items; const index = list.findIndex((item) => item.id === selectedItemId); const target = index + offset;
  ...
  const itemNavigation = <aside><StudioButton onClick={addItem} disabled={items.length >= NATIVE_COMPLETE_SENTENCES_LIMITS.items}><Plus />Add Sentence</StudioButton>{items.map((item, index) => <button type="button" key={item.id} aria-current={item.id === selectedItemId ? "true" : undefined} onClick={() => setSelectedItemId(item.id)}><strong>Sentence {index + 1}</strong><span>{authoringSentences[item.id] || item.prompt || "Untitled"}</span><code>{item.id}</code></button>)}</aside>;
  ...
  return <section className="native-activity-foundation native-single-choice-editor studio-editor">
    ...
    <StudioTabWorkspace id="native-complete-sentences-tabs" value={tab} onChange={(value) => { setTab(value); setDrawing(false); }} tabs={tabs} label="Complete the Sentences authoring modes">
      {tab === "content" ? <div className="native-single-choice-back"><section className="studio-content-panel"><StudioField label="Activity title"><input value={publicDraft.metadata.title} maxLength={300} onChange={(event) => mutatePublic((next) => { next.metadata.title = event.target.value; })} /></StudioField></section><div className="native-or-question-workspace">{itemNavigation}{selectedItem ? <section className="native-or-question-editor"><header><strong>Sentence {items.indexOf(selectedItem) + 1}</strong><div><button type="button" disabled={items.indexOf(selectedItem) === 0} onClick={() => moveItem(-1)}>Move Up</button><button type="button" disabled={items.indexOf(selectedItem) === items.length - 1} onClick={() => moveItem(1)}>Move Down</button><button type="button" onClick={removeItem}>Delete</button></div></header><StudioField label="Full sentence with one marked answer"><textarea value={markedSentence} maxLength={NATIVE_COMPLETE_SENTENCES_LIMITS.promptLength + NATIVE_COMPLETE_SENTENCES_LIMITS.answerLength + 2} aria-invalid={!markedSentenceResult?.valid || undefined} aria-describedby={`${selectedItem.id}-marked-help`} onChange={(event) => updateMarkedSentence(event.target.value)} /></StudioField><p id={`${selectedItem.id}-marked-help`} className={markedSentenceResult?.valid ? "studio-field-help" : "studio-field-error"} role={markedSentenceResult?.valid ? undefined : "alert"}>{markedSentenceResult?.valid ? "The text inside *asterisks* is stored only in the private Teacher answer; students receive an explicit blank." : markedSentenceResult?.error}</p></section> : <p>Add a sentence to begin.</p>}</div></div> : null}
      {tab === "visual" ? <section className="native-single-choice-back"><div className="native-single-choice-visual-authoring"><header><div><h3>Visual blanks</h3><p>Managed background with one source-pixel hotspot per sentence.</p></div></header><div className="studio-visual-workspace"><section className="studio-canvas-column"><div className="studio-canvas-viewport"><div className="studio-artboard-wrap"><NativeCompleteSentencesHotspotCanvas presentation={presentation} assetUrl={backgroundReference ? assetUrl(backgroundReference.assetId) : ""} items={items} selectedHotspotId={selectedHotspotId} onSelect={(id) => { setSelectedHotspotId(id); setDrawing(false); }} onCreate={createHotspot} onChange={(area) => updateHotspot((hotspot) => { hotspot.area = area; })} onDelete={deleteHotspot} drawingEnabled={drawing} /></div></div></section><aside className="studio-inspector"><label className="studio-upload-action"><Upload /><span><strong>{uploading ? "Uploading…" : backgroundReference ? "Replace background" : "Upload background"}</strong><small>PNG, JPEG or WebP</small></span><input type="file" accept="image/png,image/jpeg,image/webp" disabled={uploading} onChange={(event) => { uploadBackground(event.target.files?.[0]); event.target.value = ""; }} /></label><StudioField label="Sentence to map"><select value={drawItemId} onChange={(event) => { setDrawItemId(event.target.value); setDrawing(false); }}><option value="">Choose a sentence</option>{items.map((item, index) => <option key={item.id} value={item.id} disabled={mapped.has(item.id) && selectedHotspot?.itemId !== item.id}>Sentence {index + 1}: {item.prompt || "Untitled"}</option>)}</select></StudioField><StudioButton variant="primary" selected={drawing} disabled={!drawItemId || !backgroundReference || Boolean(selectedHotspot)} onClick={() => setDrawing((current) => !current)}>Draw blank hotspot</StudioButton>{selectedHotspot ? <><StudioField label="Hotspot binding"><select value={selectedHotspot.itemId} onChange={(event) => updateHotspot((hotspot) => { hotspot.itemId = event.target.value; })}>{items.map((item, index) => <option key={item.id} value={item.id} disabled={mapped.has(item.id) && item.id !== selectedHotspot.itemId}>Sentence {index + 1}</option>)}</select></StudioField><StudioButton variant="danger-ghost" onClick={deleteHotspot}><Trash2 />Delete Hotspot</StudioButton></> : null}</aside></div></div></section> : null}
      {tab === "answer-key" ? <div className="native-single-choice-back"><section className="studio-content-panel"><header><div><span className="studio-section-icon"><KeyRound /></span><div><h3>Private answer key</h3><p>Teacher-only answers remain outside the public activity document.</p></div></div></header></section><div className="native-or-question-workspace">{itemNavigation}{selectedItem ? <section className="native-or-question-editor"><strong>Sentence {items.indexOf(selectedItem) + 1}</strong><p>{selectedItem.prompt}</p><StudioField label="Correct answer"><input value={selectedAnswer?.text || ""} maxLength={NATIVE_COMPLETE_SENTENCES_LIMITS.answerLength} onChange={(event) => updateAnswer(event.target.value)} /></StudioField></section> : <p>Add a sentence to begin.</p>}</div></div> : null}
      ...
      {tab === "preview" ? <div className="studio-preview-panel"><header><Eye /><div><h3>Local Preview</h3><p>Preview includes the current unsaved draft. Review remains pinned to saved state.</p></div></header><div className="native-or-preview-toggle"><button type="button" aria-pressed={preview === "student"} onClick={() => setPreview("student")}>Student Preview</button><button type="button" aria-pressed={preview === "teacher"} onClick={() => setPreview("teacher")}>Teacher Preview</button></div>{preview === "student" ? <NativeCompleteSentencesStudentSurface document={publicDraft} assetUrl={assetUrl} /> : <NativeCompleteSentencesTeacherSurface publicDocument={publicDraft} teacherDocument={teacherDraft} assetUrl={assetUrl} />}</div> : null}
    </StudioTabWorkspace>
