...
import { addNativeCompleteSentencesItem, alignNativeCompleteSentencesAnswers, removeNativeCompleteSentencesItem, replaceNativeCompleteSentencesBackground } from "../../../data/native-activities/nativeCompleteSentencesAuthoring.js";
...
export function NativeCompleteSentencesEditor({ bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {}, onSaved = () => {} }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setPublicDraft(publicValue.document); setTeacherDraft(teacherValue.document); setSelectedItemId(publicValue.document.parts[0].interaction.items[0]?.id || null);
  ...
  const answer = teacherDraft?.parts[0].solution.answers.find((entry) => entry.itemId === selectedItemId) || null;
  ...
  const addItem = () => { let id; mutatePair((nextPublic, nextTeacher) => { id = addNativeCompleteSentencesItem(nextPublic, nextTeacher); }); setSelectedItemId(id); };
  ...
  return <section className="native-activity-foundation native-single-choice-editor studio-editor">
    ...
    {mode === "back" ? <div className="native-single-choice-back"><section className="studio-content-panel"><div className="studio-form-grid"><StudioField label="Activity title"><input value={publicDraft.metadata.title} maxLength={300} onChange={(event) => mutatePublic((next) => { next.metadata.title = event.target.value; })} /></StudioField><StudioField className="studio-field--wide" label="Visible instruction"><textarea value={publicDraft.metadata.visibleInstructionText} maxLength={2000} onChange={(event) => mutatePublic((next) => { next.metadata.visibleInstructionText = event.target.value; })} /></StudioField></div></section>
      <div className="native-or-question-workspace"><aside><StudioButton onClick={addItem} disabled={items.length >= NATIVE_COMPLETE_SENTENCES_LIMITS.items}><Plus />Add Sentence</StudioButton>{items.map((item, index) => <button type="button" key={item.id} aria-current={item.id === selectedItemId ? "true" : undefined} onClick={() => setSelectedItemId(item.id)}><strong>Sentence {index + 1}</strong><span>{item.prompt || "Untitled"}</span><code>{item.id}</code></button>)}</aside>{selectedItem ? <section className="native-or-question-editor"><header><strong>Sentence {items.indexOf(selectedItem) + 1}</strong><div><button type="button" disabled={items.indexOf(selectedItem) === 0} onClick={() => moveItem(-1)}>Move Up</button><button type="button" disabled={items.indexOf(selectedItem) === items.length - 1} onClick={() => moveItem(1)}>Move Down</button><button type="button" onClick={removeItem}>Delete</button></div></header><StudioField label="Sentence / prompt"><textarea value={selectedItem.prompt} maxLength={NATIVE_COMPLETE_SENTENCES_LIMITS.promptLength} onChange={(event) => mutatePublic((next) => { next.parts[0].interaction.items.find((item) => item.id === selectedItem.id).prompt = event.target.value; })} /></StudioField><StudioField label="Private correct word or phrase"><input value={answer?.text || ""} maxLength={NATIVE_COMPLETE_SENTENCES_LIMITS.answerLength} onChange={(event) => mutatePair((_public, teacher) => { teacher.parts[0].solution.answers.find((entry) => entry.itemId === selectedItem.id).text = event.target.value; })} /></StudioField></section> : <p>Add a sentence to begin.</p>}</div>
