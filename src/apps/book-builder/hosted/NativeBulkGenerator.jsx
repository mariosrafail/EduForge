...
const formats = Object.freeze({
  "mark-the-words": { label: "Mark the Words", instruction: "Number passages consecutively from 1. Wrap each correct word or expression in *asterisks*. Escape literal asterisks with \\* and backslashes with \\\\. Slashes are literal.", example: "1. I *watch* films while my watch *charges*.\n2. They *have been working* all morning." },
  "complete-sentences": {
...
export function NativeBulkGenerator({ kind, hasExistingContent, onGenerate }) {
  ...
  const blocked = !source.trim() || (kind !== "mark-the-words" && hasExistingContent && !replaceExisting);
  return <details className="native-bulk-generator">
    ...
    <div className="native-bulk-generator__body">
      ...
      {hasExistingContent ? <label className="native-bulk-generator__replace"><input type="checkbox" checked={replaceExisting} onChange={(event) => setReplaceExisting(event.target.checked)} /> <span><strong>Replace existing semantic content</strong><small>{kind === "mark-the-words" ? "Unchecked: append passages. Replacement removes existing passages, answers and word hotspots; backgrounds and media remain. You will be asked to confirm." : "Questions, items, options or words and their private answers will change. Compatible panels, assets and geometry are preserved."}</small></span></label> : null}
