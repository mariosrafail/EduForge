...
const tabs = [{ id: "front", label: "Front / Student" }, { id: "back", label: "Back / Teacher" }, { id: "preview", label: "Preview", icon: Eye }];
...
export function NativeDragDropEditor({ bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {}, onSaved = () => {} }) {
  ...
  return <section className="native-activity-foundation native-drag-drop-editor studio-editor">
    ...
    <StudioTabs value={tab} onChange={(value) => { setTab(value); setDrawingTarget(false); }} tabs={tabs} label="Drag and Drop authoring modes" />

    {tab === "front" ? <><div className="native-drag-drop-title-field"><StudioField label="Activity title"><input value={publicDraft.metadata.title} maxLength="300" onChange={(event) => mutatePublic((next) => { next.metadata.title = event.target.value; })} /></StudioField></div><div className="native-drag-drop-editor-grid" role="tabpanel">
      <aside className="native-drag-drop-editor-list"><h3>Shared word bank</h3><StudioButton onClick={addWord} disabled={interaction.words.length >= NATIVE_DRAG_DROP_LIMITS.words}><Plus aria-hidden="true" />Add word</StudioButton>{interaction.words.map((word, index) => <div className="native-drag-drop-word-row" key={word.id}><input aria-label={`Word ${index + 1}`} value={word.text} maxLength={NATIVE_DRAG_DROP_LIMITS.wordTextLength} onChange={(event) => mutatePublic((next) => { next.parts[0].interaction.words[index].text = event.target.value; })} /><button type="button" aria-label={`Move word ${index + 1} up`} disabled={!index} onClick={() => mutatePublic((next) => moveInArray(next.parts[0].interaction.words, index, -1))}>↑</button><button type="button" aria-label={`Move word ${index + 1} down`} disabled={index === interaction.words.length - 1} onClick={() => mutatePublic((next) => moveInArray(next.parts[0].interaction.words, index, 1))}>↓</button><button type="button" aria-label={`Remove word ${index + 1}`} onClick={() => deleteWord(word.id)}><Trash2 aria-hidden="true" /></button></div>)}
        <h3>Panels</h3><StudioButton onClick={addPanel} disabled={interaction.panels.length >= NATIVE_DRAG_DROP_LIMITS.panels}><Plus aria-hidden="true" />Add panel</StudioButton>{interaction.panels.map((entry, index) => <div className="native-drag-drop-panel-row" key={entry.id}><button type="button" aria-current={panel?.id === entry.id ? "true" : undefined} onClick={() => { setPanelId(entry.id); setSelection(null); setDrawingTarget(false); }}>Panel {index + 1} · {entry.images.length} image{entry.images.length === 1 ? "" : "s"} · {entry.dropTargets.length} target{entry.dropTargets.length === 1 ? "" : "s"}</button><button type="button" aria-label={`Move panel ${index + 1} up`} disabled={!index} onClick={() => movePanel(index, -1)}>↑</button><button type="button" aria-label={`Move panel ${index + 1} down`} disabled={index === interaction.panels.length - 1} onClick={() => movePanel(index, 1)}>↓</button></div>)}
      </aside>
