...
export function NativeImageEditor({ bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {}, onSaved = () => {} }) {
  ...
  return <section className="native-activity-foundation native-image-editor native-or-editor">
    <header><div><span>Native draft · {readiness.ready ? "Content complete" : "Content incomplete"}</span><h2>{publicDraft.metadata.title}</h2><p>{placementLabel} · Image</p></div><details className="builder-technical-details"><summary>Technical details</summary><dl><div><dt>Stable ID</dt><dd><code>{activityId}</code></dd></div><div><dt>Revisions</dt><dd>Public {state.publicRevision} · Teacher {state.teacherRevision}</dd></div></dl></details></header>
