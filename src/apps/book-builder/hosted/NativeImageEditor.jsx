...
import { Accessibility, Copy, Eye, FileText, ImagePlus, Layers3, LayoutPanelTop, LockKeyhole, Trash2, Upload } from "lucide-react";
...
export function NativeImageEditor({ bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {}, onSaved = () => {} }) {
  ...
  useEffect(() => {
    ...
    setPublicDraft(null); setTeacherDraft(null); setDirty(false); setSelectedId(null); onDirtyChange(false);
    Promise.all([
      ...
      <section className="studio-canvas-column" aria-label="Image canvas">
        ...
        <div className="studio-canvas-viewport"><div className="studio-artboard-wrap" style={{ width: `${zoom * 100}%` }}>
          <NativeImageSurface document={publicDraft} assetUrl={assetUrl} selectedId={selectedId} onSelect={setSelectedId} className="studio-artboard">
            {selectedImage ? <StageSelectionFrame geometry={selectedImage.area} stage={interaction.surface} label={selectedImage.altText || "Image"} locked={selectedImage.locked} preserveAspectRatio minWidth={24} minHeight={24} onChange={(area) => updateSelected((item) => { item.area = area; })} onClear={() => setSelectedId(null)} onDelete={() => setConfirmingRemove(true)} /> : null}
          </NativeImageSurface>
        </div></div>
