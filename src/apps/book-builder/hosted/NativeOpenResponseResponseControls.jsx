...
function ConfiguredAnswerFontSize({ value, presentation, onChange }) {
  ...
  return <StudioField label="Requested answer size" className="studio-quick-field studio-quick-field--wide"><input aria-label="Requested answer size" type="number" inputMode="numeric" step="1" value={draft} aria-describedby="native-or-requested-size-status" onChange={(event) => setDraft(event.target.value)} onBlur={commit} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); event.currentTarget.blur(); } if (event.key === "Escape") { event.preventDefault(); setDraft(String(value)); setMessage("Edit cancelled; the saved value is unchanged."); } }} /><small id="native-or-requested-size-status" role="status">{message}</small></StudioField>;
}
