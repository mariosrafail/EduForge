...
import { assessNativeOpenResponseReadiness, createNativeOpenResponseQuestion, nativeOpenResponseLinePositions } from "../../../data/native-activities/nativeOpenResponse.js";
...
export function NativeOpenResponseEditor({ bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {} }) {
  ...
  const updateSelectedArea = (key, raw) => {
    const value = Number(raw); if (!Number.isFinite(value) || !selectedArea) return;
  ...
  const beginDrag = (event, mode) => {
    if (!selectedArea) return; event.preventDefault(); event.currentTarget.setPointerCapture?.(event.pointerId);
  ...
  const uploadArtwork = async (file) => {
    ...
    try {
      ...
      mutatePublic((next) => {
        next.assets.push(uploaded.reference);
        next.parts[0].interaction.artwork.push({ id: artworkId, assetSlot: slot, area: { x: 160, y: 120, width: 320, height: 220 }, order: next.parts[0].interaction.artwork.length, altText: "", decorative: false, fit: "contain" });
      });
  ...
  const removeArtwork = (id) => {
    ...
    mutatePublic((next) => { const list = next.parts[0].interaction.artwork; const item = list.find((entry) => entry.id === id); next.parts[0].interaction.artwork = list.filter((entry) => entry.id !== id).map((entry, index) => ({ ...entry, order: index })); next.assets = next.assets.filter((entry) => entry.slot !== item.assetSlot); });
    setSelection(null);
  };
  ...
  return <section className="native-activity-foundation native-or-editor">
    ...
    {tab === "layout" ? <div className="native-or-layout"><div><NativeOpenResponseSurface document={publicDraft} assetUrl={previewAsset} selected={selection} onSelect={(value) => { setSelection(value); if (value.type !== "artwork") setSelectedQuestionId(value.id); }}>
      ...
      {selection?.type === "artwork" ? <ArtworkProperties item={interaction.artwork.find((item) => item.id === selection.id)} update={(mutator) => mutatePublic((next) => mutator(next.parts[0].interaction.artwork.find((item) => item.id === selection.id), next.parts[0].interaction.artwork))} remove={() => removeArtwork(selection.id)} /> : null}
    </aside></div> : null}
    {tab === "preview" ? <div className="native-or-preview"><div className="native-or-preview-toggle"><button type="button" aria-pressed={preview === "student"} onClick={() => setPreview("student")}>Student Preview</button><button type="button" aria-pressed={preview === "teacher"} onClick={() => setPreview("teacher")}>Teacher Preview</button></div><h3>{publicDraft.metadata.title}</h3>{publicDraft.metadata.visibleInstructionText ? <p>{publicDraft.metadata.visibleInstructionText}</p> : null}{preview === "student" ? <NativeOpenResponseSurface document={publicDraft} assetUrl={previewAsset} /> : <NativeOpenResponseTeacherSurface publicDocument={publicDraft} teacherDocument={teacherDraft} assetUrl={previewAsset} />}</div> : null}
...
function ArtworkProperties({ item, update, remove }) {
  if (!item) return null;
  const moveOrder = (offset) => update((target, list) => { const index = list.indexOf(target); const nextIndex = clamp(index + offset, 0, list.length - 1); if (index === nextIndex) return; [list[index], list[nextIndex]] = [list[nextIndex], list[index]]; list.forEach((entry, order) => { entry.order = order; }); });
  return <><code>{item.id}</code><label><span>Alt text</span><textarea value={item.altText} maxLength={2000} onChange={(event) => update((target) => { target.altText = event.target.value; })} /></label><label><input type="checkbox" checked={item.decorative} onChange={(event) => update((target) => { target.decorative = event.target.checked; })} /> Decorative</label><label><span>Fit</span><select value={item.fit} onChange={(event) => update((target) => { target.fit = event.target.value; })}><option value="contain">Contain</option><option value="cover">Cover</option></select></label><div><button type="button" onClick={() => moveOrder(-1)}>Send Backward</button><button type="button" onClick={() => moveOrder(1)}>Bring Forward</button></div><button type="button" onClick={remove}>Remove graphic</button></>;
}
