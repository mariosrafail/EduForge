...
import { BookOpenText, Eye, FileText, FileUp, Film, KeyRound, LayoutPanelTop, Music, Plus, Upload } from "lucide-react";
...
export function NativeListeningEditor({ bookSlug, componentSlug, activityId, placementLabel, onDirtyChange = () => {}, onSaved = () => {} }) {
  ...
  useEffect(() => {
    ...
    setState({ kind: "loading", publicRevision: 0, teacherRevision: 0, saving: false, message: "" }); setPublicDraft(null); setTeacherDraft(null); setTab("content"); setDirty(false); onDirtyChange(false);
    Promise.all([
  ...
  return <section className="native-activity-foundation native-listening-editor studio-editor studio-open-response">
    ...
    {tab === "preview" ? <section className="native-or-preview"><p><strong>Local Preview</strong> includes the current unsaved draft. Review remains pinned to saved state.</p><div className="native-or-preview-toggle"><button type="button" aria-pressed={preview === "student"} onClick={() => setPreview("student")}>Student Preview</button><button type="button" aria-pressed={preview === "teacher"} onClick={() => setPreview("teacher")}>Teacher Preview</button></div>{preview === "student" ? <NativeListeningStudentSurface document={publicDraft} assetUrl={assetUrl} /> : <NativeListeningTeacherSurface publicDocument={publicDraft} teacherDocument={teacherDraft} assetUrl={assetUrl} />}</section> : null}
