...
import TeacherProjectPresentation from "../../android-teacher-project/TeacherProjectPresentation.jsx";
...
function PreviewCanvas({ config, viewport, qaFocus, qaActive }) {
...
function Overview({ project, progress, contentProgress, dirty, buildState, onImport }) {
  const sections = [["shell", "Shell & Animation"], ["chrome", "Window Controls"], ["units", "Units"], ["editions", "Book Editions"], ["toolbar", "Teacher Toolbar"]];
  return <div className="teacher-overview"><div className="teacher-overview-metrics"><article><strong>{progress.configuredCount} / {progress.requiredCount}</strong><span>Shell assignments</span></article><article><strong>{contentProgress.entryCount}</strong><span>Page / spread entries</span></article><article><strong>{contentProgress.completeEntryCount} / {contentProgress.entryCount}</strong><span>Complete page entries</span></article><article><strong>{contentProgress.unitCountWithContent} / 10</strong><span>Units with pages</span></article></div><div className="teacher-overview-sections">{sections.map(([id, label]) => <div key={id}><span>{label}</span><strong>{progress.sections[id].configured} / {progress.sections[id].required}</strong><progress value={progress.sections[id].configured} max={progress.sections[id].required} /></div>)}</div><div className="teacher-overview-status"><strong>Revision {project.revision} · {dirty ? "Unsaved changes" : "Saved"}</strong><span>{progress.missingCount || contentProgress.incompleteEntryCount ? `${progress.missingCount} missing shell assignments and ${contentProgress.incompleteEntryCount} incomplete page entries will use grey image placeholders; missing sounds stay silent.` : buildState.job ? `Latest build: ${buildState.job.stage} (${buildState.job.status})` : "Page content is export-ready."}</span></div>{progress.configuredCount === 0 && <div className="teacher-empty-guide"><h3>Your shell is empty</h3><p>You can build immediately with placeholders, or import a prepared folder and assign the final artwork later.</p><button type="button" className="studio-button primary" onClick={onImport}><Import aria-hidden="true" />Import Assets</button></div>}</div>;
}
...
export function TeacherProjectEditor({ projectId, writeEnabled }) {
  ...
  const load = useCallback(() => {
    ...
    requestTeacherProject(projectId, { signal: controller.signal }).then(({ project }) => setState({ status: "ready", project, draftName: project.displayName, draftShell: structuredClone(project.shell), draftContent: structuredClone(project.content), error: null })).catch((error) => { if (error.name !== "AbortError") setState({ status: "error", project: null, draftName: "", draftShell: null, draftContent: null, error }); });
  ...
  const { project, draftShell, draftContent } = state;
  ...
  const updateContent = (content) => setState((current) => ({ ...current, draftContent: content }));
  ...
  const dirty = state.draftName !== project.displayName || JSON.stringify(draftShell) !== JSON.stringify(project.shell) || JSON.stringify(draftContent) !== JSON.stringify(project.content);
  ...
  const renderSection = () => {
    if (section === "overview") return <Overview project={project} progress={progress} contentProgress={contentProgress} dirty={dirty} buildState={buildState} onImport={() => setImportOpen(true)} />;
    if (section === "pages") return <TeacherProjectPagesEditor project={project} content={draftContent} urls={urls} usage={usage} writeEnabled={writeEnabled} onContentChange={updateContent} onProjectChange={(nextProject) => setState((current) => ({ ...current, project: nextProject }))} />;
  ...
  return <main className="teacher-project-editor" id="main-content">
    <div className="teacher-project-editor-top">
      <header className="teacher-project-editor-header">
        ...
        <div className="teacher-project-editor-actions"><button className="studio-button secondary" type="button" disabled={!writeEnabled} onClick={() => setImportOpen(true)}><Import aria-hidden="true" />Import Assets</button><span className={`teacher-project-inline-status ${saveState.error ? "is-error" : ""}`} role={saveState.error ? "alert" : "status"}>{saveState.message}</span><button className="studio-button primary" type="button" disabled={!writeEnabled || saveState.pending || !dirty} onClick={save}><Save aria-hidden="true" />{saveState.pending ? "Saving…" : "Save"}</button><button className="studio-button secondary teacher-preview-toggle" type="button" aria-pressed={previewOpen} onClick={() => previewOpen ? closePreview() : setPreviewOpen(true)}>{previewOpen ? <EyeOff aria-hidden="true" /> : <Eye aria-hidden="true" />}{previewOpen ? "Hide Preview" : "Preview"}</button><button className="studio-button secondary" type="button" disabled={!writeEnabled || dirty || buildBusy} onClick={startExport} title={dirty ? "Save first" : "Export debug APK; missing graphics use grey placeholders"}><Download aria-hidden="true" />Export APK</button><button className="studio-button secondary" type="button" disabled={!writeEnabled || dirty || buildBusy} onClick={chooseRunTarget} title={dirty ? "Save first" : "Build, install and launch on Android; missing graphics use grey placeholders"}><Play aria-hidden="true" />Run</button></div>
      </header>
