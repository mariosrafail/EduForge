...
function ReviewItems({ selectedGroup, onPage, writeEnabled, onDecide }) {
  ...
  return (
    <section className="studio-review-items" aria-labelledby="review-items-title"><div className="studio-section-heading"><div><span className="studio-eyebrow">Selected group</span><h3 id="review-items-title">{selectedGroup.id.replaceAll("_", " ")}</h3></div><Badge tone="warning">{selectedGroup.pagination.total.toLocaleString()} items</Badge></div>{selectedGroup.items.map((item) => <article key={item.id}><div className="studio-review-item-heading"><div><strong>{item.reasonCode.replaceAll("_", " ")}</strong><small>{item.category} · {item.severity}</small></div><div className="studio-inline-badges">{item.blocking && <Badge tone="danger">Blocking</Badge>}<Badge tone={item.effectiveStatus === "open" ? "warning" : item.effectiveStatus === "stale_resolution" ? "danger" : "positive"}>{item.effectiveStatus}</Badge>{writeEnabled && <button type="button" className="studio-button secondary" onClick={() => onDecide(item)}>Decide item</button>}</div></div><p>{item.explanation}</p><dl><div><dt>Source</dt><dd>{item.sourceRelativeLocator}</dd></div><div><dt>Recommended domain decision</dt><dd>{item.suggestedDecisionKind.replaceAll("_", " ")}</dd></div><div><dt>Dependencies</dt><dd>{item.dependencyCount}</dd></div><div><dt>Generated status</dt><dd>{item.generatedStatus}</dd></div></dl></article>)}<Pagination pagination={selectedGroup.pagination} onPage={onPage} /></section>
  );
}

export function ReviewQueueView({ projectId, authoring }) {
  ...
  return (
    <div className="studio-view-content">
      <div className="studio-view-heading"><div><span className="studio-eyebrow">Generated queue with effective overlay</span><h2>Review Queue</h2><p>Generated items stay immutable; approved decisions compute effective status locally.</p></div><Badge tone={authoring.writeEnabled ? "positive" : "warning"}>{authoring.writeEnabled ? "Single-item decisions" : "Read-only"}</Badge></div>
      ...
      <div className="studio-boundary-note"><AlertOctagon aria-hidden="true" /><p>Cluster grouping remains read-only. Milestone 4B1 permits only one explicit review item at a time.</p></div>
