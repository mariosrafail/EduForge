...
export function ComponentsView({ projectId, routeQuery, authoring }) {
  ...
  const resource = useStudioResource(`/projects/${encodeURIComponent(projectId)}/components`, query, JSON.stringify(query));
  ...
  return <div className="studio-view-content">
    <div className="studio-view-heading"><div><span className="studio-eyebrow">Normalized hierarchy</span><h2>Components</h2><p>Friendly logical roles remain an overlay on stable raw source components.</p></div><Badge tone={authoring.writeEnabled ? "positive" : "warning"}>{authoring.writeEnabled ? "Single-item decisions" : "Review only"}</Badge></div>
    <div className="studio-filter-grid" aria-label="Component filters">
      ...
      <Field label="Effective role"><select value={filters.role} onChange={(event) => update("role", event.target.value)}><option value="">All roles</option>{data.filters.roles.map((role) => <option key={role}>{role}</option>)}</select></Field>
    ...
    {data.items.length ? <div className="studio-table-scroll"><table className="studio-table"><caption className="sr-only">Component candidates</caption><thead><tr><th>Logical component</th><th>Detected / effective role</th><th>Grouping</th><th>Units / groups</th><th>Pages</th><th>Activities</th><th>Reviews</th><th>Decision state</th><th>Related evidence</th></tr></thead><tbody>{data.items.map((item) => <tr key={item.candidateId}><td><strong>{item.displayName || item.name}</strong><small>Raw source: {item.name} · {item.sourceRelativeLocator}</small></td><td>{item.detectedRole || "Unresolved"}<small>Effective: {item.effectiveRole || "Unresolved"}</small></td><td>{item.groupingKind?.replaceAll("_", " ") || "unresolved"}</td><td>{item.unitGroups?.length ?? item.unitCount}</td><td>{item.pageSpreadCount}</td><td>{item.hasActivities ? item.objectCount : 0}</td><td>{item.unresolvedReviewCount ?? 0}</td><td><Badge tone={item.stale ? "danger" : item.decisionState === "approved" ? "positive" : "warning"}>{item.stale ? "stale" : item.decisionState}</Badge></td><td><div className="studio-row-actions">{authoring.writeEnabled && <button className="studio-icon-link" type="button" onClick={() => setEditing(item)}>Decide</button>}<a className="studio-icon-link" href={projectHash(projectId, "pages", { component: item.componentKey || item.name })} aria-label={`Review pages for ${item.displayName || item.name}`}>Pages <ArrowRight aria-hidden="true" /></a><a className="studio-icon-link" href={projectHash(projectId, "activities", { component: item.componentKey || item.name })} aria-label={`Review activities for ${item.displayName || item.name}`}>Activities <ArrowRight aria-hidden="true" /></a><a className="studio-icon-link" href={projectHash(projectId, "reviews", { component: item.componentKey || item.name })} aria-label={`Review queue for ${item.displayName || item.name}`}>Reviews <ArrowRight aria-hidden="true" /></a></div></td></tr>)}</tbody></table></div> : <StudioEmpty title="No component candidates match">Adjust the filters to see another candidate group.</StudioEmpty>}
    ...
    {editing && <DecisionDrawer projectId={projectId} expectedRevision={authoring.revision} target={{ targetId: editing.candidateId, label: editing.displayName || editing.name, sourceRelativeLocator: editing.sourceRelativeLocator }} kinds={[{ kind: "component_role", label: "Component role", values: COMPONENT_ROLES, initialValue: COMPONENT_ROLES.includes(editing.effectiveRole) ? editing.effectiveRole : "students_book", detectedValue: editing.detectedRole, currentDecision: editing.decision }]} onCommitted={authoring.onCommitted} onClose={() => setEditing(null)} />}
  </div>;
}
