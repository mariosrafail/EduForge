...
export function ComponentsView({ projectId, routeQuery }) {
  ...
  return (
    <div className="studio-view-content">
      ...
      {data.items.length ? <div className="studio-table-scroll"><table className="studio-table"><caption className="sr-only">Component candidates</caption><thead><tr><th>Source component</th><th>Role proposal</th><th>Confidence</th><th>Units</th><th>Parts</th><th>Objects</th><th>Page spreads</th><th>Review state</th><th>Related evidence</th></tr></thead><tbody>{data.items.map((item) => <tr key={item.candidateId}><td><strong>{item.name}</strong><small>{item.sourceRelativeLocator}</small></td><td>{item.proposedSemanticRole}</td><td>{item.confidence === null ? "—" : `${Math.round(item.confidence * 100)}%`}</td><td>{item.unitCount}</td><td>{item.partCount}</td><td>{item.objectCount}</td><td>{item.pageSpreadCount}</td><td><Badge tone={item.reviewState === "approved" ? "positive" : "warning"}>{item.reviewState}</Badge></td><td><div className="studio-row-actions"><a className="studio-icon-link" href={projectHash(projectId, "pages", { component: item.name })} aria-label={`Review pages for ${item.name}`}>Pages <ArrowRight aria-hidden="true" /></a><a className="studio-icon-link" href={projectHash(projectId, "activities", { component: `component:${item.name}` })} aria-label={`Review activities for ${item.name}`}>Activities <ArrowRight aria-hidden="true" /></a></div></td></tr>)}</tbody></table></div> : <StudioEmpty title="No component candidates match">Adjust the filters to see another candidate group.</StudioEmpty>}
