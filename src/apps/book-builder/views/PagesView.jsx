...
function PageInspector({ projectId, page }) {
  ...
  return (
    <section className="studio-page-inspector" aria-labelledby="page-inspector-title">
      ...
      <div className="studio-page-layout">
        ...
        <aside className="studio-page-details"><div className="studio-inline-badges"><Badge tone={page.exactCardinality ? "positive" : "warning"}>{page.exactCardinality ? "Exact cardinality" : "Mismatch"}</Badge><Badge>{page.hotspotCount} hotspots</Badge></div><dl className="studio-definition-list"><div><dt>Identity</dt><dd>{page.sourceRelativeIdentity}</dd></div><div><dt>HD / SD</dt><dd>{page.variants.map((item) => item.quality).join(" / ") || "Unavailable"}</dd></div><div><dt>Dimensions</dt><dd>{variant ? `${variant.width} × ${variant.height}` : "Unavailable"}</dd></div><div><dt>Printed page</dt><dd>{page.printedPage.value ?? "Unresolved"} {page.printedPage.confidence !== null && `(${Math.round(page.printedPage.confidence * 100)}%)`}</dd></div><div><dt>Unresolved regions</dt><dd>{page.unresolvedHotspotCount}</dd></div></dl><h4>Accessible hotspot candidates</h4>{page.hotspots.length ? <ol className="studio-hotspot-list">{page.hotspots.map((item, index) => <li key={item.candidateId} tabIndex={0}><span>{index + 1}</span><div><strong>Object {item.targetObject ?? "unresolved"}</strong><small>{item.geometry ? "Normalized geometry available" : "Geometry unavailable"} · {item.reviewState}</small></div></li>)}</ol> : <p className="studio-muted">No hotspot candidates were detected for this page.</p>}</aside>
      </div>
    </section>
  );
}

export function PagesView({ projectId, routeQuery }) {
