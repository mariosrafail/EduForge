...
export function ActivitiesView({ projectId, routeQuery, authoring }) {
  ...
  useEffect(() => setFilters((current) => ({ ...current, component: routeQuery.get("component") || "", unit: routeQuery.get("unit") || "", part: routeQuery.get("part") || "", activityId: routeQuery.get("activityId") || "", page: 1 })), [routeQuery]);
  ...
  const navigate = (key, value) => {
    const next = { component: key === "component" ? value : filters.component, unit: key === "component" ? "" : key === "unit" ? value : filters.unit, part: filters.part, activityId: key === "activityId" ? value : key === "component" || key === "unit" ? "" : filters.activityId };
    window.location.hash = projectHash(projectId, "activities", next).slice(1);
  };
  ...
  return <div className="studio-view-content">
    ...
    <div className="studio-master-detail"><section aria-label="Activity candidate list"><div className="studio-table-scroll"><table className="studio-table studio-selectable-table"><thead><tr><th>Hierarchy / effective type</th><th>Detected / effective disposition</th><th>Content</th><th>Questions</th><th>Options</th><th>Media</th><th>Reviews</th></tr></thead><tbody>{data.items.map((item) => <tr key={item.activityId} data-selected={data.selected?.activityId === item.activityId}><td><button type="button" onClick={() => navigate("activityId", item.activityId)}><strong>{item.hierarchy.componentDisplayName} · {item.hierarchy.groupLabel}</strong><small>{item.detectedType} → {item.effectiveType} · Part {item.part}</small></button></td><td>{item.detectedDisposition}<small>Effective: {item.effectiveDisposition}</small></td><td><Badge tone={item.contentCounts?.staleOverrides ? "danger" : item.contentCounts?.missingFields ? "warning" : "positive"}>{item.effectiveContentCompleteness}</Badge></td><td>{item.questionCount}</td><td>{item.optionCount}</td><td>{item.mediaCount}</td><td>{item.reviewCount}</td></tr>)}</tbody></table></div><Pagination pagination={data.pagination} onPage={(page) => update("page", page)} /></section><ActivityDetails activity={data.selected} notice={data.notice} writeEnabled={authoring.writeEnabled} onClassify={setClassification} onEditContent={setContentEditing} /></div>
