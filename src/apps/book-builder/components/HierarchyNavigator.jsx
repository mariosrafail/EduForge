...
export function HierarchyNavigator({ filters, component, unit, onComponentChange, onUnitChange, compact = true }) {
  ...
  const hierarchyInSync = !component || filters.selectedComponentKey === component;
  const unitOptions = hierarchyInSync ? filters.unitOptions || [] : [];
  ...
  return <section className={`studio-hierarchy-navigator${compact ? " compact" : ""}`} aria-label="Book hierarchy navigation">
    ...
    <Field label="Unit / group"><select value={hierarchyInSync ? unit : ""} disabled={!hierarchyInSync || !filters.unitFilterEnabled} onChange={(event) => onUnitChange(event.target.value)}><option value="">{selectedComponent ? `All ${selectedComponent.groupingKind === "numbered_units" ? "Units" : "groups"}` : "Select a component first"}</option>{unitOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></Field>
